...
<cfscript>
	...
	//}
	//if ( left(userFilesPath,1) neq "/" and left(userFilesPath,1) neq "http://") {
	//	userFilesPath = "/" & userFilesPath;
	//}
...
userFilesServerPath = config.serverPath & fs; 
...
<cfcatch>
  	<cfset session.currentPath="#userFilesServerPath##url.type#">
			...
			<cfscript>
				...
				while( i lte qDir.recordCount ) {
					...
					if( not listFind(".,..", qDir.name[i]) )
						folders = folders & '<Folder name="#XMLFormat(qDir.name[i])#" />';
					i=i+1;
				}
