...
<cfoutput>
	...
	<cfif attributes.property neq "fileid" or (attributes.property eq "fileid"  and attributes.bean.getType() neq 'File') >
		<a class="btn download-file" onclick="return confirmDialog('#application.rbFactory.getKeyValue(session.rb,'sitemanager.downloadconfirm')#',function(){location.href='#application.configBean.getContext()#/index.cfm/_render/file/?fileid=#attributes.bean.getvalue(attributes.property)#&method=attachment&size=source';});"><i class="icon-download"></i></a>		 	
	<cfelse>
		<a id="mura-download-locked" <cfif not attributes.locked> style="display:none"</cfif> class="btn download-file" onclick="return confirmDialog('#application.rbFactory.getKeyValue(session.rb,'sitemanager.downloadconfirm')#',function(){location.href='#application.configBean.getContext()#/index.cfm/_render/file/?fileid=#attributes.bean.getvalue(attributes.property)#&method=attachment&size=source';});"><i class="icon-download"></i> #application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.download')#</a>
		<div id="mura-download-unlocked" class="btn-group"<cfif attributes.locked> style="display:none"</cfif>>
			...
			<ul class="dropdown-menu">
				<!-- dropdown menu links -->
				<li><a href="##" onclick="return confirmDialog('#application.rbFactory.getKeyValue(session.rb,'sitemanager.downloadconfirm')#',function(){location.href='#application.configBean.getContext()#/index.cfm/_render/file/?fileid=#attributes.bean.getvalue(attributes.property)#&method=attachment&size=source';});"><i class="icon-download"></i> #application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.download')#</a></li>
