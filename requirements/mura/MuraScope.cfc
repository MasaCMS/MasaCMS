	...
	<cfif not isObject(event("contentRenderer"))>
		...
		<cfelseif len(event('siteid'))>
			...
			<cfif fileExists(expandPath(siteConfig().getThemeIncludePath()) & "/contentRenderer.cfc" )>
				...
				<cfset var $variables=themeRenderer.getAllValues()>
				<cfset var key=''>
				<cfloop collection="#$variables#" item="key">
					<cfset siteRenderer.injectMethod('#key#',$variables[key],'variables')>
				</cfloop>
				<cfloop collection="#themeRenderer#" item="key">
					<cfset siteRenderer.injectMethod('#key#',themeRenderer[key],'this')>
				</cfloop>
			</cfif>
