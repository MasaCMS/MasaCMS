...
<cfif not listFind("Portal,Gallery",request.contentBean.getType())>
	...
	<div id="svComments">
		...
		<cfif rsComments.recordcount>
			...
			<cfoutput query="rsComments"  startrow="#request.startrow#">
				...
				<dl class="#class#" id="comment-#rscomments.commentid#">
					<dt>
						<cfif rsComments.url neq ''>
							<a href="#rsComments.url#" target="_blank">#htmleditformat(rsComments.name)#</a>
						<cfelse>
							#htmleditformat(rsComments.name)#
						</cfif>
						<cfif request.isEditor and rsComments.email neq ''>
		...
		<form id="postcomment" method="post" name="addComment" action="?nocache=1" onsubmit="return validate(this);" >
			<fieldset>
