						...
						<cfloop list="#variables.data.fieldnames#" index="variables.f">
							...
							<td>
								<cfif findNoCase('attachment',variables.f) and isValid("UUID",variables.fvalue)>
									<a  href="http://#application.settingsManager.getSite(session.siteid).getDomain()##application.configBean.getServerPort()##application.configBean.getContext()#/index.cfm/_render/file//index.cfm?fileID=#variables.fvalue#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.viewattachment')#</a>
								<cfelse>
