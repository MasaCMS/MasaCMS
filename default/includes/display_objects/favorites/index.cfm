...
<cfset variables.$.siteConfig('AssetPath')rbFactory=getSite().getRBFactory()/>
...
<cfif len(getPersonalizationID())>
	<!--- list the favorites --->
	<cfset variables.rsFavorites = application.favoriteManager.getInternalContentFavorites(getPersonalizationID(), variables.$.event('siteID')) />
	<ul id="favoriteList">
	<cfif variables.rsFavorites.recordCount>
		...
		<cfloop query="variables.rsFavorites" startrow="1" endrow="5">
			...
			<cfset variables.contentLink = "<a href='#contentLink#'>#HTMLEditFormat(variables.rsFavorites.menutitle)#</a>" />
			<li id="favorite#variables.lid#"><a href="" onclick="return deleteFavorite('#variables.favoriteID#', 'favorite#lid#');" title="#xmlformat(variables.$.rbKey('favorites.removefromfavorites'))#" class="remove">[-]</a> #variables.contentLink#</li>
		</cfloop>
	...
	<cfelse>
		<li id="favoriteTip" class="defaultMsg">#variables.$.rbKey('favorites.nofavorites')#</li>	</cfif>
	</ul>
...
<div id="svPageTools">
	...
	<ul>
		...
		<li id="addFavorite" style="#addFavoriteStyle#"><a href="" onclick="return saveFavorite('#variables.userID#', '#variables.siteID#', '#JSStringFormat(variables.menuTitle)#', '#variables.contentID#', '#variables.favoriteType#')">#variables.$.rbKey('favorites.addtofavorites')#</a></li>
