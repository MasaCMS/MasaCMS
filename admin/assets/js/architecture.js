...
var siteManager={
	...
	if(document.contentForm.title.value == ''){
	 ...
	 if(typeof(this.hasFileLock) != 'undefined'
	 	&& !this.fileLockConfirmed 
	  	&& this.hasFileLock
		&& $("#file").val() != ''){
	 	 confirmDialog(this.unlockfileconfirm, 
				 					function() {
			 						//alert('true')
...
*/
	...
	loadObjectClass: function(siteid,classid,subclassid,contentid,parentid,contenthistid)	{
		...
		$.get(url + "?" + pars, 
			function(data) {
				...
				siteManager.availableObjectValidate=function(){return true};
			}
		);
		return false;
	},
		...
		$.get(url + "?" + pars, 
				function(data) {
					siteManager.loadSiteManagerInTab(
						function(){siteManager.loadSiteManager(siteManager.copySiteID,parentID,'00000000000000000000000000000000000','','','',1);});
				}
		);
	},
	...
	resetAvailableObject: function(){
		...
		this.availableObjectValidate=function(){return true};
	},
	...
	initConfiguratorParams: function(){
		...
		$("#availableObjectParams").find(".objectParam").bind(
			"change",
			function(){
				siteManager.updateAvailableObject();
		});
