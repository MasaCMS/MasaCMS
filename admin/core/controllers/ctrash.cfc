...
component extends="controller" output="false" {
	...
	public function empty(rc) output=false {
		if ( arguments.rc.$.validateCSRFTokens(context='emptytrash' & arguments.rc.siteID) ) {
	...
	public function restore(rc) output=false {
		// CVE-2025-55044: Validate CSRF token before processing trash restore
		if ( !arguments.rc.$.validateCSRFTokens(context='restore' & arguments.rc.objectID) ) {
			arguments.rc.alert = "Invalid security token. Restore operation blocked.";
			variables.fw.redirect(action="cTrash.list", append="siteID,keywords,pageNum", path="./");
			return;
		}
