...
<cfif FindNoCase('Opera','#CGI.HTTP_USER_AGENT#') LESS THAN 1>
	...
	<script type="text/javascript" src="#application.configBean.getContext()#/admin/assets/js/frontendtools.js.cfm?siteid=#tempEncodeForURL(variables.$.event('siteid'))#&contenthistid=#$.content('contenthistid')#&coreversion=#application.coreversion#&showInlineEditor=#getShowInlineEditor()#&cacheid=#createUUID()#"></script>
			...
			<div id="frontEndTools" style="display: #Cookie.fetDisplay#">	
								...
								<ul class="dropdown-menu">
									<cfif (request.r.perm  eq 'editor' or listFind(session.mura.memberships,'S2')) and not variables.$.siteConfig('EnforceChangesets')>
										<li>
											...
											<cfif $.content().requiresApproval()>
												#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.sendforapproval"))#
											<cfelse>
												#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.publish"))#
											</cfif>
											</a>
										</li>
									</cfif>
									<cfif listFindNoCase('editor,author',request.r.perm) or listFind(session.mura.memberships,'S2') >
										<li>
											<a class="mura-inline-save" data-approved="0" data-changesetid="">
												<i class="icon-edit"></i>  
												#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savedraft"))#
											</a>
										</li>
									</cfif>
									<cfif variables.$.siteConfig('HasChangesets') and (request.r.perm  eq 'editor' or listFind(session.mura.memberships,'S2')) >
										<li class="dropdown-submenu">
											...
											#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savetochangeset"))#</a>			
											...
											<ul class="dropdown-menu">
												...
												<cfloop condition="changesets.hasNext()">
													...
													<li>
														<a class="mura-inline-save" data-approved="0" data-changesetid="#changeset.getChangesetID()#">#tempEncodeForHTML(changeset.getName())#</a>
													</li>
												</cfloop>
												<cfelse>
													<li>
														<a class="mura-inline-cancel">#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.noneavailable"))#</a>
													</li>
												</cfif>
											</ul>
										</li>
									</cfif>
									<li><a class="mura-inline-cancel"><i class="icon-ban-circle"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.cancel"))#</a></li>
								</ul>
							</li>
						</cfif>
					</ul>
					...
					<cfif $.siteConfig('HasChangeSets')>
						...
						<cfelse>
							...
							<ul id="tools-changesets">
								...
								<li id="cs-title" class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown"><i>CS</i><cfif request.muraChangesetPreview>#tempEncodeForHTML(previewData.name)#<cfif isDate(previewData.publishDate)> (#LSDateFormat(previewData.publishDate,session.dateKeyFormat)#)</cfif><cfelse>None Selected</cfif><b class="caret"></b></a>
									<ul class="dropdown-menu">
										<li><a href="./?changesetid=">None</a></li>
										<cfloop query="rsChangesets">
										<li><a href="./?changesetid=#rschangesets.changesetid#">
												#tempEncodeForHTML(rsChangesets.name)#
								...
								<li class="dropdown">
									<a class="dropdown-toggle" data-toggle="dropdown" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'changesets.assignments'))#"><i class="icon-list"></i></a>
									<cfif request.muraChangesetPreview>
										<ul class="dropdown-menu">
										<cfif changesetMembers.hasNext()>
											<cfloop condition="changesetMembers.hasNext()">
											<cfset changesetMember=changesetMembers.next()>
											<li><a href="#changesetMember.getURL()#">#tempEncodeForHTML(changesetMember.getMenuTitle())#</a></li>
											</cfloop>
										<cfelse>
								...
								<cfif request.muraChangesetPreview and structKeyExists(previewData.previewmap,$.content("contentID")) >
									<cfif previewData.previewmap[$.content("contentID")].changesetID eq previewData.changesetID>
										<li>
											<a href="" data-toggle="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'changesets.content.in'))#">
												<i class="icon-check"></i>
												 <!---#application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"changesets.previewnodemembership"),'<strong>"#tempEncodeForHTMLAttribute(previewData.previewmap[$.content("contentID")].changesetName)#"</strong>')#--->
											</a>
										</li>
									<cfelse>
										<li>
											<a href="" data-toggle="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'changesets.content.dependent'))#">
								...
								<cfelse>
									<li>
										<a href="" data-toggle="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'changesets.content.notin'))#">
				...
				<cfif $.currentUser().isLoggedIn()>
					<ul id="tools-user">
						...
						<li id="adminWelcome"><i class="icon-user"></i> #tempEncodeForHTML("#session.mura.fname# #session.mura.lname#")#</li>
					</ul>
				</cfif>
