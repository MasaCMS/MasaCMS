		...
		<cfdefaultcase>
			...
			<cfif isDefined('rc.feedBean') and not rc.feedBean.getIsNew()>
				<cfif rc.feedBean.getType() eq 'Local'>
					<a class="btn" title="#application.rbFactory.getKeyValue(session.rb,'collections.view')#" href="http://#application.settingsManager.getSite(rc.siteid).getDomain()##application.configBean.getServerPort()##application.configBean.getContext()#/index.cfm/_feed/#rc.feedBean.getSiteID()#/?feedID=#rc.feedBean.getfeedID()#" target="_blank"><i class="icon-rss"></i> #application.rbFactory.getKeyValue(session.rb,'collections.viewfeed')#</a>
				<cfelse>
