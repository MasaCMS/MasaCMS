...
<cfif not isBlocked>
	<form novalidate="novalidate" id="loginForm" name="frmLogin" method="post" action="index.cfm" onsubmit="return submitForm(this);">
		...
		<input name="returnUrl" type="hidden" value="#tempEncodeForHTMLAttribute(rc.returnURL)#">
		...
		<div class="control-group">
			...
			<p class="help-block">
				<cfif rc.status eq 'sendLogin'>
				  <cfset msg=application.userManager.sendLoginByEmail('#rc.email#','','#tempEncodeForURL("#listFirst(cgi.http_host,":")##cgi.SCRIPT_NAME#")#')>
				<cfif left(msg,2) eq "No">

				#tempEncodeForHTML(application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"login.noaccountexists"),rc.email))#		
				<cfelseif left(msg,4) eq "Your">
				#tempEncodeForHTML(application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"login.messagesent"),rc.email))#
				<cfelse>	#tempEncodeForHTML(application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"login.invalidemail"),rc.email))#
				</cfif>
				<cfelse>
