...
<h3>#encodeForHTML(chain.getName())#</h3>
<p>#encodeForHTML(chain.getDescription())#</p>

<cfif members.hasNext()>
	<cfloop condition="members.hasNext()">
		<cfset member=members.next()>
		<h4>#members.getRecordIndex()#. #encodeForHTML(member.getGroup().getGroupName())#</h4>
		    ...
		    <cfif requests.hasNext()>
		     <cfloop condition="requests.hasNext()">
		        ...
		        <tr>  
		          ...
		          <cfif hasChangesets>
		          	<td>
		          		<cfif isDate(item.getchangesetPublishDate())><a href="##" rel="tooltip" title="#encodeForHTMLAttribute(LSDateFormat(item.getchangesetPublishDate(),"short"))#"> <i class="icon-calendar"></i></a></cfif>
		          		<a href="./?muraAction=cChangesets.assignments&siteID=#item.getSiteID()#&changesetID=#item.getChangeSetID()#">#encodeForHTML(item.getChangesetName())#</a></td>
		          </cfif>
		          <td>#encodeForHTML(item.getLastUpdateBy())#</td>
