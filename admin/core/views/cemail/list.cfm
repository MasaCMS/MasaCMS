...
<button type="button" class="btn" onclick="submitForm(document.forms.form1);"><i class="icon-filter"></i> #application.rbFactory.getKeyValue(session.rb,'email.filter')#</button>
	  ...
	  <table id="metadata" class="mura-table-grid">
        <tr> 
		  ...
		  <cfoutput>
		  <th class="var-width"><a href="./?muraAction=cEmail.list&siteid=#encodeForURL(rc.siteid)#&orderBy=subject&direction=#subjectDirection#">#application.rbFactory.getKeyValue(session.rb,'email.subject')#</a></th>
		  ...
		  <th><a href="./?muraAction=cEmail.list&siteid=#encodeForURL(rc.siteid)#&orderBy=createdDate&direction=#createdDateDirection#">#application.rbFactory.getKeyValue(session.rb,'email.datecreated')#</a></th>
          <th><a href="./?muraAction=cEmail.list&siteid=#encodeForURL(rc.siteid)#&orderBy=deliveryDate&direction=#deliveryDateDirection#">#application.rbFactory.getKeyValue(session.rb,'email.deliverydate')#</a></th>
          <th><a href="./?muraAction=cEmail.list&siteid=#encodeForURL(rc.siteid)#&orderBy=status&direction=#statusDirection#">#application.rbFactory.getKeyValue(session.rb,'email.status')#</a></th>
          <th>&nbsp;</th>
		  </cfoutput>
        </tr>
		  ...
		  <cfoutput>
			  <cfloop query="rc.rslist" startRow="#startRow#" endRow="#endRow#"> 
				  ...
				  <cfset bounces=application.emailManager.getStat(rc.rslist.emailid,'bounce')/>
					<tr> 
					  <td class="var-width"><a title="Edit" href="./?muraAction=cEmail.edit&emailid=#rc.rslist.emailid#&siteid=#encodeForURL(rc.siteid)#">#rc.rslist.subject#</a></td>
					  <td>#clicks#</td>
					  <td>#opens#</td>
					  <td>#bounces#</td>
					  <td>#sent#</td>
					  ...
					  <td class="actions"><ul><li class="edit"><a title="#application.rbFactory.getKeyValue(session.rb,'email.edit')#" href="./?muraAction=cEmail.edit&emailid=#rc.rslist.emailid#&siteid=#encodeForURL(rc.siteid)#"><i class="icon-pencil"></i></a></li><li class="download"><a title="#application.rbFactory.getKeyValue(session.rb,'email.download')#" href="./?muraAction=cEmail.download&emailID=#rc.rsList.emailID#&siteid=#encodeForURL(rc.siteid)#"><i class="icon-download-alt"></i></a></li><li class="delete"><a title="#application.rbFactory.getKeyValue(session.rb,'email.delete')#" href="./?muraAction=cEmail.update&action=delete&emailid=#rc.rslist.emailid#&siteid=#encodeForURL(rc.siteid)#" onclick="return confirmDialog('#encodeForJavascript(application.rbFactory.getKeyValue(session.rb,'email.deleteconfirm'))#',this.href)"><i class="icon-remove-sign"></i></a></li></ul></td>
					</tr>
			  </cfloop>
	  ...
	  <cfif rc.rslist.recordcount gt recordsPerPage>
		...
		<cfloop from="1" to="#Ceiling(rc.rslist.recordcount/recordsPerPage)#" index="i">
			<cfif i eq rc.page>
				#i#
			<cfelse>
				<a href="./?muraAction=cEmail.list&siteid=#encodeForURL(rc.siteid)#&page=#i#&direction=#rc.direction#&orderBy=#rc.orderBy#">#i#</a>
			</cfif>
		</cfloop> 
	  </cfif>
	  </cfoutput>