...
#evaluate(round((opens/sent)*100))#%
        ...
        <li><a href="./?muraAction=cEmail.showReturns&emailid=#rc.emailid#&siteid=#tempEncodeForURL(rc.siteid)#"><strong>#application.rbFactory.getKeyValue(session.rb,'email.userswhoclicked')#:</strong></a> #clicks# (
          <cfif sent gt 0>
#evaluate(round((clicks/sent)*100))#%
        ...
        <li><a href="./?muraAction=cEmail.showReturns&emailid=#rc.emailid#&siteid=#tempEncodeForURL(rc.siteid)#"><strong>#application.rbFactory.getKeyValue(session.rb,'email.uniqueclicks')#:</strong></a> #uniqueClicks# </li>
        <li><a href="./?muraAction=cEmail.showReturns&emailid=#rc.emailid#&siteid=#tempEncodeForURL(rc.siteid)#"><strong>#application.rbFactory.getKeyValue(session.rb,'email.totalclicks')#:</strong></a> #totalClicks# </li>
        <li><a href="./?muraAction=cEmail.showBounces&emailid=#rc.emailid#&siteid=#tempEncodeForURL(rc.siteid)#"><strong>Bounces:</strong></a> #bounces# (
          <cfif sent gt 0>
...
<form novalidate="novalidate" action="./?muraAction=cEmail.update&siteid=#tempEncodeForURL(rc.siteid)#" method="post" name="form1" onSubmit="return false;">
...
<div class="tabbable tabs-left mura-ui">
    ...
    <div id="emailContent" class="tab-pane fade">
      ...
      <div class="controls">
          <input type="text" class="span12" name="subject" value="#tempEncodeForHTMLAttribute(rc.emailBean.getsubject())#"  required="true" message="#application.rbFactory.getKeyValue(session.rb,'email.subjectrequired')#">
        </div>
      </div>
        ...
        <label class="control-label">
         ...
         <div class="controls">
          <input type="text" class="span12" name="fromLabel" value="#iif(rc.emailBean.getFromLabel() neq '',de("#tempEncodeForHTMLAttribute(rc.emailBean.getFromLabel())#"),de("#tempEncodeForHTMLAttribute(application.settingsManager.getSite(rc.siteid).getsite())#"))#"  required="true" message="The 'From Label' form field is required" >
          </div>
         </div>
        </div>
...
#application.pluginManager.renderScripts("onHTMLEdit",rc.siteid,pluginEvent,rsPluginScripts)#
		...
		{
		fckEditor.config["CustomConfigurationsPath"]=tempEncodeForURL('#application.settingsManager.getSite(rc.siteid).getThemeAssetPath()#/js/fckconfig.js.cfm?EditorType=Email');
		}
		
		fckEditor.create(); // create the editor.
	</cfscript>
              <cfelse>
              <textarea name="bodyHTML" id="bodyHTML"><cfif len(rc.emailBean.getBodyHTML())>#tempEncodeForHTML(rc.emailBean.getBodyHTML())#<cfelse><p></p></cfif>
</textarea>
        ...
        <div class="controls">
          <textarea name="bodyText" id="textEditor">#tempEncodeForHTML(rc.emailBean.getbodytext())#</textarea>
          </div>
        </div>
...
<!---Delivery Options--->
	...
	<cfif showDelete>
	<button type="button" class="btn toggle" onClick="emailManager.validateEmailForm('delete', '#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'email.deleteconfirm'))#');"><i class="icon-remove"></i> #application.rbFactory.getKeyValue(session.rb,'email.delete')#</button>
	</cfif>
	<button type="button" class="btn toggle" onClick="emailManager.validateEmailForm('#formAction#', '#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'email.saveconfirm'))#')"><i class="icon-check"></i> #application.rbFactory.getKeyValue(session.rb,'email.save')#</button>
        ...
        <div style="display:none" id="scheduler">
            ...
            <cfset deliveryDate = rc.emailBean.getDeliveryDate()>
	          ...
	          <div class="scheduler-actions">
		          <button type="button" class="btn" onClick="emailManager.validateScheduler('#formAction#', '#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'email.pleaseenterdate'))#', 'deliveryDate');"><i class="icon-check"></i> #application.rbFactory.getKeyValue(session.rb,'email.save')#</button>
