...
<cfoutput>
<form novalidate="novalidate" action="./?muraAction=cPublicUsers.updateAddress&userid=#tempEncodeForURL(rc.userid)#&routeid=#rc.routeid#&siteid=#tempEncodeForURL(rc.siteid)#" method="post" enctype="multipart/form-data" name="form1" onsubmit="return validate(this);"  autocomplete="off" >
	...
	<h2>#tempEncodeForHTML(rc.userBean.getFname())# #tempEncodeForHTML(rc.userBean.getlname())#</h2>
      ...
      <div class="controls"><textarea id="hours" rows="6" class="span6" name="hours" >#tempEncodeForHTML(rsAddress.hours)#</textarea></div>
    </div>
...
initTabs(Array("#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'user.basic'))#","#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'user.extendedattributes'))#"),0,0,0);
</script>	
</cfif>
</div>
