...
<cfoutput>
<form novalidate="novalidate" action="./?muraAction=cPublicUsers.updateAddress&userid=#encodeForURL(rc.userid)#&routeid=#rc.routeid#&siteid=#encodeForURL(rc.siteid)#" method="post" enctype="multipart/form-data" name="form1" onsubmit="return validate(this);"  autocomplete="off" >
	...
	<h2>#encodeForHTML(rc.userBean.getFname())# #encodeForHTML(rc.userBean.getlname())#</h2>
		<div class="fieldset-wrap">
		<div class="fieldset">
		<div class="control-group">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.addressname')#</label>
      <div class="controls"><input id="addressName" name="addressName" type="text" value="#encodeForHTMLAttribute(rsAddress.addressName)#"  class="span6"></div>
    </div>
		
		<div class="control-group">
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.address1')#</label>
      <div class="controls"><input id="address1" name="address1" type="text" value="#encodeForHTMLAttribute(rsAddress.address1)#"  class="span12"></div>
    </div>
		
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.address2')#</label>
      <div class="controls"><input id="address2" name="address2" type="text" value="#encodeForHTMLAttribute(rsAddress.address2)#"  class="span12"></div>
    </div>
    </div>
      ...
      <div class="controls"><input id="city" name="city" type="text" value="#encodeForHTMLAttribute(rsAddress.city)#" class="span12"></div>
    </div>
		
		<div class="span1">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.state')#</label>
      <div class="controls"><input id="state" name="state" type="text" value="#encodeForHTMLAttribute(rsAddress.state)#" class="span12"></div>
    </div>
		
		<div class="span2">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.zip')#</label>
      <div class="controls"><input id="zip" name="zip" type="text" value="#encodeForHTMLAttribute(rsAddress.zip)#" class="span12"></div>
    </div>
		
		<div class="span4">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.country')#</label>
      <div class="controls"><input id="country" name="country" type="text" value="#encodeForHTMLAttribute(rsAddress.country)#" class="span12"></div>
    </div>
    </div>
      ...
      <div class="controls"><input id="phone" name="phone" type="text" value="#encodeForHTMLAttribute(rsAddress.phone)#" class="span12"></div>
    </div>
		
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.fax')#</label>
      <div class="controls"><input id="fax" name="fax" type="text" value="#encodeForHTMLAttribute(rsAddress.fax)#" class="span12"></div>
    </div>
    </div>
      ...
      <div class="controls"><input id="addressURL" name="addressURL" type="text" value="#encodeForHTMLAttribute(rsAddress.addressURL)#" class="span12"></div>
    </div>
		
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.email')#</label>
      <div class="controls"><input id="addressEmail" name="addressEmail" validate="email" message="#application.rbFactory.getKeyValue(session.rb,'user.emailvalidate')#" type="text" value="#encodeForHTMLAttribute(rsAddress.addressEmail)#" class="span12"></div>
    </div>
    </div>
		
		<div class="control-group">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.hours')#</label>
      <div class="controls"><textarea id="hours" rows="6" class="span6" name="hours" >#encodeForHTML(rsAddress.hours)#</textarea></div>
    </div>
...
<cfoutput><cfset style=extendSetBean.getStyle()/><cfif not len(style)><cfset started=true/></cfif>
	...
	<cfloop from="1" to="#arrayLen(attributesArray)#" index="a">	
      ...
      <label class="control-label">
		...
		<a href="##" rel="tooltip" title="#encodeForHTMLAttribute(attributeBean.gethint())#">#attributeBean.getLabel()# <cfif attributeBean.getType() IS "Hidden"><strong>[Hidden]</strong></cfif> <i class="icon-question-sign"></i></a>
		<cfelse>
...
initTabs(Array("#encodeForJavascript(application.rbFactory.getKeyValue(session.rb,'user.basic'))#","#encodeForJavascript(application.rbFactory.getKeyValue(session.rb,'user.extendedattributes'))#"),0,0,0);
</script>	
</cfif>
</div>
	...
	<div class="form-actions">
		<cfif rc.addressid eq ''>
           ...
           <cfelse>
				...
				<input type="button" class="btn" onclick="submitForm(document.forms.form1,'delete','#encodeForJavascript(application.rbFactory.getKeyValue(session.rb,'user.deleteaddressconfirm'))#');" value="#application.rbFactory.getKeyValue(session.rb,'user.delete')#" />
           </cfif>
    </div>
