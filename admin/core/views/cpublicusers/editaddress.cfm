...
<cfoutput>
<form novalidate="novalidate" action="./?muraAction=cPublicUsers.updateAddress&userid=#esapiEncode('url',rc.userid)#&routeid=#rc.routeid#&siteid=#esapiEncode('url',rc.siteid)#" method="post" enctype="multipart/form-data" name="form1" onsubmit="return validate(this);"  autocomplete="off" >
	...
	<h2>#esapiEncode('html',rc.userBean.getFname())# #esapiEncode('html',rc.userBean.getlname())#</h2>
      ...
      <div class="controls"><input id="addressName" name="addressName" type="text" value="#esapiEncode('html_attr',rsAddress.addressName)#"  class="span6"></div>
    </div>
		
		<div class="control-group">
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.address1')#</label>
      <div class="controls"><input id="address1" name="address1" type="text" value="#esapiEncode('html_attr',rsAddress.address1)#"  class="span12"></div>
    </div>
		
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.address2')#</label>
      <div class="controls"><input id="address2" name="address2" type="text" value="#esapiEncode('html_attr',rsAddress.address2)#"  class="span12"></div>
    </div>
    </div>
      ...
      <div class="controls"><input id="city" name="city" type="text" value="#esapiEncode('html_attr',rsAddress.city)#" class="span12"></div>
    </div>
		
		<div class="span1">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.state')#</label>
      <div class="controls"><input id="state" name="state" type="text" value="#esapiEncode('html_attr',rsAddress.state)#" class="span12"></div>
    </div>
		
		<div class="span2">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.zip')#</label>
      <div class="controls"><input id="zip" name="zip" type="text" value="#esapiEncode('html_attr',rsAddress.zip)#" class="span12"></div>
    </div>
		
		<div class="span4">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.country')#</label>
      <div class="controls"><input id="country" name="country" type="text" value="#esapiEncode('html_attr',rsAddress.country)#" class="span12"></div>
    </div>
    </div>
      ...
      <div class="controls"><input id="phone" name="phone" type="text" value="#esapiEncode('html_attr',rsAddress.phone)#" class="span12"></div>
    </div>
		
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.fax')#</label>
      <div class="controls"><input id="fax" name="fax" type="text" value="#esapiEncode('html_attr',rsAddress.fax)#" class="span12"></div>
    </div>
    </div>
      ...
      <div class="controls"><input id="addressURL" name="addressURL" type="text" value="#esapiEncode('html_attr',rsAddress.addressURL)#" class="span12"></div>
    </div>
		
		<div class="span6">
      <label class="control-label">#application.rbFactory.getKeyValue(session.rb,'user.email')#</label>
      <div class="controls"><input id="addressEmail" name="addressEmail" validate="email" message="#application.rbFactory.getKeyValue(session.rb,'user.emailvalidate')#" type="text" value="#esapiEncode('html_attr',rsAddress.addressEmail)#" class="span12"></div>
    </div>
    </div>
...
<cfoutput><cfset style=extendSetBean.getStyle()/><cfif not len(style)><cfset started=true/></cfif>
	...
	<cfloop from="1" to="#arrayLen(attributesArray)#" index="a">	
      ...
      <label class="control-label">
		...
		<a href="##" rel="tooltip" title="#esapiEncode('html_attr',attributeBean.gethint())#">#attributeBean.getLabel()# <cfif attributeBean.getType() IS "Hidden"><strong>[Hidden]</strong></cfif> <i class="icon-question-sign"></i></a>
		<cfelse>
		#attributeBean.getLabel()# <cfif attributeBean.getType() IS "Hidden"><strong>[Hidden]</strong></cfif>
		</cfif>
		<cfif attributeBean.getType() eq "File" and len(attributeValue) and attributeValue neq 'useMuraDefault'> <a href="#application.configBean.getContext()#/tasks/render/file/index.cfm?fileID=#attributeValue#" target="_blank">[Download]</a> <input type="checkbox" value="true" name="extDelete#attributeBean.getAttributeID()#"/> Delete</cfif>
		</label>
...
initTabs(Array("#esapiEncode('javascript',application.rbFactory.getKeyValue(session.rb,'user.basic'))#","#esapiEncode('javascript',application.rbFactory.getKeyValue(session.rb,'user.extendedattributes'))#"),0,0,0);
</script>	
</cfif>
</div>
	...
	<div class="form-actions">
		<cfif rc.addressid eq ''>
           ...
           <cfelse>
				...
				<input type="button" class="btn" onclick="submitForm(document.forms.form1,'delete','#esapiEncode('javascript',application.rbFactory.getKeyValue(session.rb,'user.deleteaddressconfirm'))#');" value="#application.rbFactory.getKeyValue(session.rb,'user.delete')#" />
           </cfif>
    </div>
