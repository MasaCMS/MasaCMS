	...
	<div class="fieldset">
		<cfif rsNonDefault.recordcount>	
			<div class="control-group">
		     	...
		     	<div class="controls">
		     		<select name="subtype" class="dropdown" onchange="userManager.resetExtendedAttributes('#rc.userBean.getUserID()#','2',this.value,'#userPoolID#','#application.configBean.getContext()#','#application.settingsManager.getSite(rc.siteid).getThemeAssetPath()#');">
		...
		<div class="control-group">
			...
			<cfif rsAddresses.recordcount>
				...
				<tr>
					...
					<cfif rsAddresses.isPrimary neq 1><li class="delete"><a title="Delete" href="index.cfm?muraAction=cPublicUsers.updateAddress&userid=#URLEncodedFormat(rc.userid)#&action=delete&siteid=#URLEncodedFormat(rc.siteid)#&routeID=#URLEncodedFormat(rc.routeid)#&addressID=#rsAddresses.addressID#" onclick="return confirmDialog('#jsStringFormat(application.rbFactory.getKeyValue(session.rb,'user.deleteaddressconfirm'))#',this.href);">#application.rbFactory.getKeyValue(session.rb,'user.delete')#</a></li><cfelse><li class="delete disabled">#application.rbFactory.getKeyValue(session.rb,'user.delete')#</li></cfif></ul></td>
				</tr>
				</cfloop>
				</table>
			<cfelse>
...
<div id="tabGroupmemberships" class="tab-pane fade">
		...
		<cfelse>
		<div class="control-group"><div class="cotnrols"><p class="notice">#application.rbFactory.getKeyValue(session.rb,'user.nogroups')#</p></div></div>
		</cfif>
	</div>
</div>
