...
<input type="button" class="btn" onclick="window.location='./?muraAction=cPublicUsers.advancedSearch&siteid=#encodeForURL(rc.siteid)#&newSearch=true'" value="#application.rbFactory.getKeyValue(session.rb,'user.advanced')#" /><
--->
</form>
...
<a class="btn" href="./?muraAction=cPublicUsers.edituser&siteid=#encodeForURL(rc.siteid)#&userid="><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,'user.addmember')#</a>
<a class="btn" href="./?muraAction=cPublicUsers.editgroup&siteid=#encodeForURL(rc.siteid)#&userid="><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,'user.addgroup')#</a>

 <cfif listFind(session.mura.memberships,'Admin;#application.settingsManager.getSite(rc.siteid).getPrivateUserPoolID()#;0') or listFind(session.mura.memberships,'S2')>
    <a class="btn" href="./?muraAction=cPerm.module&contentid=00000000000000000000000000000000008&siteid=#encodeForURL(rc.siteid)#&moduleid=00000000000000000000000000000000008"><i class="icon-group"></i> #application.rbFactory.getKeyValue(session.rb,'user.permissions')#</a>
  </cfif>
</div>
...
<h2>#application.rbFactory.getKeyValue(session.rb,'user.usergroups')#</h2>
        ...
        <table class="mura-table-grid">
                ...
                <cfoutput query="rc.rsgroups"> 
                  <tr> 
                    <td class="var-width"> 
                      <a title="#application.rbFactory.getKeyValue(session.rb,'user.edit')#" href="./?muraAction=cPublicUsers.editgroup&userid=#rc.rsgroups.UserID#&siteid=#encodeForURL(rc.siteid)#">#encodeForHTML(rc.rsgroups.groupname)#</a> (<cfif isNumeric(rc.rsgroups.counter)>#rc.rsgroups.counter#<cfelse>0</cfif>) </td>
                    <td> 
