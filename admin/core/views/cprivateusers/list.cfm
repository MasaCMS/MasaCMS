...
<cfoutput>
		  ...
		  <form class="form-inline" novalidate="novalidate" action="./?muraAction=cPrivateUsers.search&siteid=#tempEncodeForURL(rc.siteid)#" method="get" name="form1" id="siteSearch">
    ...
    <input type="hidden" name='siteid' value="#tempEncodeForHTMLAttribute(rc.siteid)#"/>
      ...
      <div id="nav-module-specific" class="btn-group"><a class="btn" href="./?muraAction=cPrivateUsers.edituser&siteid=#tempEncodeForURL(rc.siteid)#&userid="><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,'user.addmember')#</a>
      <a class="btn" href="./?muraAction=cPrivateUsers.editgroup&siteid=#tempEncodeForURL(rc.siteid)#&userid="><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,'user.addgroup')#</a>
      </div>
      ...
      <h2>#application.rbFactory.getKeyValue(session.rb,'user.adminusergroups')#</h2>
        ...
        <table class="mura-table-grid">
                ...
                <cfoutput query="rc.rsgroups"> 
                  <tr> 
                    <td class="var-width"> 
                      <a title="Edit" href="./?muraAction=cPrivateUsers.editgroup&userid=#tempEncodeForURL(rc.rsgroups.userID)#&siteid=#tempEncodeForURL(rc.siteid)#">#tempEncodeForHTML(rc.rsgroups.groupname)#</a>  (<cfif isNumeric(rc.rsgroups.counter)>#rc.rsgroups.counter#<cfelse>0</cfif>) 
                 </td>
                    <td> 
