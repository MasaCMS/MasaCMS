...
<ul class="navTask nav nav-pills"><li><a href="javascript:intuserselect('#rc.userid#',1,<cfif listFind(session.mura.memberships,'S2')>1<cfelse>0</cfif>,'#rc.siteid#')">#application.rbFactory.getKeyValue(session.rb,'user.addadminusertogroup')#</a></li></ul></cfif>
</cfoutput>
...
<dd><input type="text" class="text" name="groupname" value="#HTMLEditFormat(rc.userBean.getgroupname())#"  required="true" message="#application.rbFactory.getKeyValue(session.rb,'user.groupnamerequired')#"></dd>
<dt>#application.rbFactory.getKeyValue(session.rb,'user.email')#</dt>
<dd><input type="text" class="text" name="email" value="#HTMLEditFormat(rc.userBean.getemail())#"></dd>
...
<input type="hidden" name="siteid" value="#HTMLEditFormat(rc.siteid)#"></div></cfoutput></form>
</cfdefaultcase>
</cfswitch>
	...
	<cfif rc.nextN.numberofpages gt 1> 
		<p class="moreResults"><cfoutput>#application.rbFactory.getKeyValue(session.rb,'user.moreresults')#:
			...
			<cfloop from="#rc.nextn.firstPage#"  to="#rc.nextN.lastPage#" index="i"><cfif rc.nextN.currentpagenumber eq i> <strong>#i#</strong> <cfelse> <a href="index.cfm?muraAction=cPrivateUsers.editgroup&startrow=#evaluate('(#i#*#rc.nextN.recordsperpage#)-#rc.nextN.recordsperpage#+1')#&userid=#URLEncodedFormat(rc.userid)#&siteid=#URLEncodedFormat(rc.siteid)#">#i#</a> </cfif></cfloop>
