	...
	<div class="tab-content">
		<div id="tabCurrentsites" class="tab-pane fade"> 
			...
			<form novalidate="novalidate" name="form1" id="form1" action="./?muraAction=csettings.list" method="post">
					...
					<tr>
						...
						<td>
							<cfif len(rc.rsSites.domain)>
								#tempEncodeForHTML(rc.rsSites.domain)#
								<cfelse>
								-
							</cfif>
						</td>
						...
						<td class="actions"><ul <cfif application.configBean.getMode() eq 'Staging'>class="three"<cfelse>class="two"</cfif>>
								...
								<cfif application.configBean.getMode() eq 'Staging'>
									<cfif application.configBean.getValue('deployMode') eq "bundle">
										<li class="deploy"><a href="?muraAction=cSettings.deploybundle&siteid=#rc.rsSites.siteid#" onclick="return confirmDialog('Deploy #tempEncodeForJavascript(rc.rsSites.site)# to production?',this.href);" title="Deploy">Deploy</a></li>
									<cfelse>
										<li class="deploy"><a href="?muraAction=cSettings.list&action=deploy&siteid=#rc.rsSites.siteid#" onclick="return confirmDialog('Deploy #tempEncodeForJavascript(rc.rsSites.site)# to production?',this.href);" title="Deploy">Deploy</a></li>
									</cfif>
								</cfif>
								<cfif rc.rsSites.siteid neq 'default'>
									<li class="delete"><a title="Delete" href="##" onclick="confirmDialog('#tempEncodeForJavascript("WARNING: A deleted site and all of its files cannot be recovered. Are you sure that you want to delete the site named '#Ucase(rc.rsSites.site)#'?")#',function(){actionModal('./?muraAction=cSettings.updateSite&action=delete&siteid=#rc.rsSites.siteid##rc.$.renderCSRFTokens(context=rc.rssites.siteid,format='url')#')});return false;"><i class="icon-remove-sign"></i></a></li>
									<cfelse>
									<li class="delete disabled"><i class="icon-remove-sign"></i></li>
								</cfif>
								<!---<li class="export"><a title="Export" href="./?muraAction=cArch.exportHtmlSite&siteid=#rc.rsSites.siteid#" onclick="return confirm('Export the #tempEncodeForJavascript("'#rc.rsSites.site#'")# Site?')">Export</a></li>--->
							</ul></td>
					</tr>
					</cfoutput>
				</table>
				...
				<cfoutput>
					<input type="hidden" name="siteSortBy" value="#tempEncodeForHTMLAttribute(rc.siteSortBy)#" />
		...
		<h2>Current Plugins</h2>
			<table class="mura-table-grid">
				...
				<cfif rc.rsPlugins.recordcount>
					...
					<tr>
						<td class="var-width"><a class="alt" title="view" href="#application.configBean.getContext()#/plugins/#rc.rsPlugins.directory#/">#tempEncodeForHTML(rc.rsPlugins.name)#</a></td>
						<td>#tempEncodeForHTML(rc.rsPlugins.directory)#</td>
						<td>#tempEncodeForHTML(rc.rsPlugins.category)#</td>
						<td>#tempEncodeForHTML(rc.rsPlugins.version)#</td>
						<td><a class="alt" href="#rc.rsPlugins.providerurl#" target="_blank">#tempEncodeForHTML(rc.rsPlugins.provider)#</a></td>
