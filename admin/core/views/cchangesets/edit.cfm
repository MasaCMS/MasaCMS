...
<form novalidate="novalidate" action="index.cfm?muraAction=cChangesets.save&siteid=#URLEncodedFormat(rc.siteid)#" method="post" name="form1" onsubmit="return validate(this);">
...
<div class="control-group">
  <label class="control-label">
  	<a href="##" class="tooltip">#application.rbFactory.getKeyValue(session.rb,'changesets.publishdate')#<span>#application.rbFactory.getKeyValue(session.rb,"tooltip.changesetpublishdate")#</span></a>
  	</label>
  <div class="controls">
  	<cfif rc.changeset.getPublished()>
...
<input type="button" class="submit btn btn-primary" onclick="submitForm(document.forms.form1,'add');" value="#application.rbFactory.getKeyValue(session.rb,'changesets.add')#" /><input type=hidden name="changesetID" value="">
<cfelse>
<input type="button" class="submit btn btn-danger" value="#application.rbFactory.getKeyValue(session.rb,'changesets.delete')#" onclick="confirmDialog('#jsStringFormat(application.rbFactory.getKeyValue(session.rb,'changesets.deleteconfirm'))#','index.cfm?muraAction=cChangesets.delete&changesetID=#rc.changeset.getchangesetID()#&siteid=#URLEncodedFormat(rc.changeset.getSiteID())#')" /> 
<input type="button" class="submit btn btn-primary" onclick="submitForm(document.forms.form1,'update');" value="#application.rbFactory.getKeyValue(session.rb,'changesets.update')#" />
<cfif not rc.changeset.getPublished()>
<input type="button" class="submit" value="#application.rbFactory.getKeyValue(session.rb,'changesets.publishnow')#" onclick="confirmDialog('#jsStringFormat(application.rbFactory.getKeyValue(session.rb,'changesets.publishnowconfirm'))#','index.cfm?muraAction=cChangesets.publish&changesetID=#rc.changeset.getchangesetID()#&siteid=#URLEncodedFormat(rc.changeset.getSiteID())#')" /> 
</cfif>
