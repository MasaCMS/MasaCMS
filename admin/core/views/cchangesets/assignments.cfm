...
<h2>#encodeForHTML(rc.changeset.getName())#</h2>

<cfif not rc.changeset.getPublished()>
<p><i class="icon-link"></i> <a title="Change Set Name" href="##" onclick="return preview('#encodeForJavascript(rc.previewLink)#','');">#encodeForHTML(rc.previewLink)#</a></p>
</cfif>

<form class="form-inline separate" novalidate="novalidate" id="assignmentSearch" name="assignmentSearch" method="get">
	<input name="keywords" placeholder="Keywords" value="#encodeForHTMLAttribute(rc.keywords)#" type="text" class="text" maxlength="50" />
	...
	<input type="hidden" name="changesetID" value="#encodeForHTMLAttribute(rc.changesetID)#">
</form>
