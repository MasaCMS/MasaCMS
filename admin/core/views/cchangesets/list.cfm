...
<div class="row-fluid">
	...
	<tr>
		<td class="var-width"><a title="Edit" href="./?muraAction=cChangesets.assignments&changesetID=#rc.changeset.getchangesetID()#&siteid=#URLEncodedFormat(rc.siteID)#&categoryid=#URLEncodedFormat(rc.categoryid)#&tags=#URLEncodedFormat(rc.tags)#">#HTMLEditFormat(rc.changeset.getName())#</a></td>
		...
		<td class="actions">
			<ul>
				<li class="edit"><a title="#application.rbFactory.getKeyValue(session.rb,'changesets.edit')#" href="./?muraAction=cChangesets.edit&changesetID=#rc.changeset.getchangesetID()#&siteid=#URLEncodedFormat(rc.changeset.getSiteID())#&categoryid=#URLEncodedFormat(rc.categoryid)#&tags=#URLEncodedFormat(rc.tags)#"><i class="icon-pencil"></i></a></li>
				...
				<li class="delete"><a title="#application.rbFactory.getKeyValue(session.rb,'changesets.delete')#" href="./?muraAction=cChangesets.delete&changesetID=#rc.changeset.getchangesetID()#&siteid=#URLEncodedFormat(rc.changeset.getSiteID())#&categoryid=#URLEncodedFormat(rc.categoryid)#&tags=#URLEncodedFormat(rc.tags)##rc.$.renderCSRFTokens(context=rc.changeset.getChangesetID(),format='url')#" onclick="return confirmDialog('#jsStringFormat(application.rbFactory.getKeyValue(session.rb,'changesets.deleteconfirm'))#',this.href)"><i class="icon-remove-sign"></i></a></li>
			</ul>
		</td>
	</tr></cfloop>
	<cfelse>
	...
	<cfif rc.changesets.pageCount() gt 1>
		...
		<div class="clearfix mura-results-wrapper">
			...
			<cfif rc.changesets.getPageIndex() lt rc.changesets.pagecount()>
				<li>
					<a href="./?muraAction=cChangesets.list&page=#evaluate('#rc.changesets.getPageIndex()#+1')#&siteid=#URLEncodedFormat(rc.siteid)#&keywords=#URLEncodedFormat(rc.keywords)#&startdate=#URLEncodedFormat(rc.startdate)#&stopdate=#URLEncodedFormat(rc.stopdate)#&categoryid=#URLEncodedFormat(rc.categoryid)#&tags=#URLEncodedFormat(rc.tags)#">&laquo;&nbsp;#application.rbFactory.getKeyValue(session.rb,'user.next')#</a>
				</li>
			</cfif>
			</ul>
			</div>
	...
	<form novalidate="novalidate" name="searchFrm" class="form-inline" onsubmit="return validate(this);">
		...
		<div class="module well">
			<h3>#application.rbFactory.getKeyValue(session.rb,"params.keywords")#</h3>
		  	<input name="keywords" value="#HTMLEditFormat(rc.keywords)#" type="text" class="span12"  maxlength="50" />
		</div>
		...
		<cfif len($.event('categoryID') & $.event('tags') & $.event('keywords') & $.event('startdate') & $.event('stopdate'))>
	  	<input type="button" class="btn" name="removeFilter" value="#application.rbFactory.getKeyValue(session.rb,"sitemanager.removefilter")#" onclick="location.href='./?siteID=#urlEncodedFormat($.event('siteid'))#&muraAction=cChangesets.list'"/>
	  	</cfif>
