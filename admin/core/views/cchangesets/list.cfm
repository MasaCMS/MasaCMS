...
<form class="form-inline" novalidate="novalidate" id="changesetSearch" name="changesetSearch" method="get">
	...
	<input name="keywords" value="#tempEncodeForHTMLAttribute(rc.keywords)#" type="text" class="text" maxlength="50" />
...
<div class="row-fluid">
	...
	<tr>
		<td class="var-width"><a title="Edit" href="./?muraAction=cChangesets.assignments&changesetID=#rc.changeset.getchangesetID()#&siteid=#tempEncodeForURL(rc.siteID)#&categoryid=#tempEncodeForURL(rc.categoryid)#&tags=#tempEncodeForURL(rc.tags)#">#tempEncodeForHTML(rc.changeset.getName())#</a></td>
		...
		<td class="actions">
			<ul>
				<li class="edit"><a title="#application.rbFactory.getKeyValue(session.rb,'changesets.edit')#" href="./?muraAction=cChangesets.edit&changesetID=#rc.changeset.getchangesetID()#&siteid=#tempEncodeForURL(rc.changeset.getSiteID())#&categoryid=#tempEncodeForURL(rc.categoryid)#&tags=#tempEncodeForURL(rc.tags)#"><i class="icon-pencil"></i></a></li>
				<cfif rc.changeset.getPublished()>
					<li class="preview disabled"><i class="icon-globe"></i></li>
				<cfelse>
					<li class="preview"><a title="#application.rbFactory.getKeyValue(session.rb,'changesets.preview')#" href="##" onclick="return preview('http://#application.settingsManager.getSite(rc.siteid).getDomain()##application.configBean.getServerPort()##application.configBean.getContext()##$.getURLStem(rc.siteid,"")#?changesetID=#tempEncodeForJavascript(rc.changeset.getChangesetID())#','');"><i class="icon-globe"></i></a></li>
				</cfif>
				<li class="change-sets"><a title="#application.rbFactory.getKeyValue(session.rb,'changesets.assignments')#" href="./?muraAction=cChangesets.assignments&changesetID=#rc.changeset.getchangesetID()#&siteid=#tempEncodeForURL(rc.changeset.getSiteID())#&categoryid=#tempEncodeForURL(rc.categoryid)#"><i class="icon-reorder"></i></a></li>
				<li class="delete"><a title="#application.rbFactory.getKeyValue(session.rb,'changesets.delete')#" href="./?muraAction=cChangesets.delete&changesetID=#rc.changeset.getchangesetID()#&siteid=#tempEncodeForURL(rc.changeset.getSiteID())#&categoryid=#tempEncodeForURL(rc.categoryid)#&tags=#tempEncodeForURL(rc.tags)##rc.$.renderCSRFTokens(context=rc.changeset.getChangesetID(),format='url')#" onclick="return confirmDialog('#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'changesets.deleteconfirm'))#',this.href)"><i class="icon-remove-sign"></i></a></li>
			</ul>
		</td>
	</tr></cfloop>
	<cfelse>
	...
	<cfif rc.changesets.pageCount() gt 1>
		...
		<div class="clearfix mura-results-wrapper">
			...
			<li>
				<a href="./?muraAction=cChangesets.list&page=#evaluate('#rc.changesets.getPageIndex()#-1')#&siteid=#tempEncodeForURL(rc.siteid)#&keywords=#tempEncodeForURL(rc.keywords)#&startdate=#tempEncodeForURL(rc.startdate)#&stopdate=#tempEncodeForURL(rc.stopdate)#&categoryid=#tempEncodeForURL(rc.categoryid)#&tags=#tempEncodeForURL(rc.tags)#">&laquo;&nbsp;#application.rbFactory.getKeyValue(session.rb,'user.prev')#</a>
			</li>
			</cfif>
			<cfloop from="1" to="#rc.changesets.pageCount()#" index="i">
				...
				<cfelse> 
					<li>
						<a href="./?muraAction=cChangesets.list&page=#i#&siteid=#tempEncodeForURL(rc.siteid)#&keywords=#tempEncodeForURL(rc.keywords)#&startdate=#tempEncodeForURL(rc.startdate)#&stopdate=#tempEncodeForURL(rc.stopdate)#&categoryid=#tempEncodeForURL(rc.categoryid)#"&tags=#tempEncodeForURL(rc.tags)#>#i#</a>
					</li>
				</cfif>
			</cfloop>
			<cfif rc.changesets.getPageIndex() lt rc.changesets.pagecount()>
				<li>
					<a href="./?muraAction=cChangesets.list&page=#evaluate('#rc.changesets.getPageIndex()#+1')#&siteid=#tempEncodeForURL(rc.siteid)#&keywords=#tempEncodeForURL(rc.keywords)#&startdate=#tempEncodeForURL(rc.startdate)#&stopdate=#tempEncodeForURL(rc.stopdate)#&categoryid=#tempEncodeForURL(rc.categoryid)#&tags=#tempEncodeForURL(rc.tags)#">&laquo;&nbsp;#application.rbFactory.getKeyValue(session.rb,'user.next')#</a>
				</li>
			</cfif>
			</ul>
			</div>
	...
	<form novalidate="novalidate" name="searchFrm" class="form-inline" onsubmit="return validate(this);">
		...
		<div class="module well">
			<h3>#application.rbFactory.getKeyValue(session.rb,"params.keywords")#</h3>
		  	<input name="keywords" value="#tempEncodeForHTMLAttribute(rc.keywords)#" type="text" class="span12"  maxlength="50" />
		</div>
		...
		<cfif len($.event('categoryID') & $.event('tags') & $.event('keywords') & $.event('startdate') & $.event('stopdate'))>
	  	<input type="button" class="btn" name="removeFilter" value="#application.rbFactory.getKeyValue(session.rb,"sitemanager.removefilter")#" onclick="location.href='./?siteID=#tempEncodeForURL($.event('siteid'))#&muraAction=cChangesets.list'"/>
	  	</cfif>
