...
<form class="fieldset-wrap" novalidate="novalidate" action="./?muraAction=cAdvertising.updateCampaign&siteid=#tempEncodeForURL(rc.siteid)#" name="form1"  method="post" onsubmit="return validate(this);">
<div class="fieldset"> 
<div class="control-group">
	<label class="control-label">#application.rbFactory.getKeyValue(session.rb,'advertising.name')#</label>
	<div class="controls"><input name="name" type="text" required="true" message="The 'Name' field is required." value="#tempEncodeForHTMLAttribute(rc.campaignBean.getName())#" maxlength="50">
	</div>
</div>
...
<div class="control-group">
	...
	<div class="controls"><textarea name="notes" class="span12" rows="6">#tempEncodeForHTML(rc.campaignBean.getNotes())#</textarea>	
	</div>
</div>
</div>
...
<cfelse>
	<input type="button" class="btn" onclick="submitForm(document.forms.form1,'delete','#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'advertising.deletecampaignconfirm'))#');" value="#application.rbFactory.getKeyValue(session.rb,'advertising.delete')#" />
...
<input type="hidden" name="userID" value="#tempEncodeForHTMLAttribute(rc.userid)#">
</form>
</cfoutput>
<cfif rc.campaignid neq ''>
	...
	<a class="btn" href="./?muraAction=cAdvertising.editPlacement&campaignid=#tempEncodeForURL(rc.campaignid)#&placementid=&siteid=#tempEncodeForURL(rc.siteid)#&userid=#tempEncodeForURL(rc.userid)#">#application.rbFactory.getKeyValue(session.rb,'advertising.addplacement')#</a>
	</div>
	...
	<cfif rc.rsPlacements.recordcount>
		...
		<cfoutput query="rc.rsPlacements">
		 <cfsilent>
			...
			<tr>
				<td class="var-width"><a href="./?muraAction=cAdvertising.editAdZone&siteid=#tempEncodeForURL(rc.siteid)#&adzoneid=#rc.rsplacements.adzoneid#">#rc.rsPlacements.Adzone#</a></td>
				<td><a href="./?muraAction=cAdvertising.editCreative&userid=#tempEncodeForURL(rc.userid)#&creativeid=#rc.rsplacements.creativeid#&siteid=#tempEncodeForURL(rc.siteid)#">#rc.rsPlacements.creative#</a></td>
				...
				<td class="actions"><ul>
				<li class="edit"><a title="#application.rbFactory.getKeyValue(session.rb,'user.edit')#" href="./?muraAction=cAdvertising.editPlacement&userid=#tempEncodeForURL(rc.userid)#&siteid=#tempEncodeForURL(rc.siteid)#&campaignid=#rc.campaignID#&placementid=#rc.rsplacements.placementid#"><i class="icon-pencil"></i></a></li>
				<li class="view-report"><a title="#application.rbFactory.getKeyValue(session.rb,'advertising.viewplacementreport')#" href="./?muraAction=cAdvertising.viewReportByPlacement&placementid=#rc.rsPlacements.placementid#&campaignid=#rc.campaignid#&userid=#tempEncodeForURL(rc.userid)#&siteid=#tempEncodeForURL(rc.siteid)#"><i class="icon-bar-chart"></i></a></li>
				<li class="delete"><a title="#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'user.delete'))#" href="./?muraAction=cAdvertising.updatePlacement&action=delete&campaignid=#rc.campaignid#&siteid=#tempEncodeForURL(rc.siteid)#&userid=#tempEncodeForURL(rc.userid)#&placementid=#rc.rsplacements.placementid#" onclick="return confirmDialog('#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'advertising.deleteplacementconfirm'))#',this.href)"><i class="icon-remove-sign"></i></a></li></ul>
				</td></tr>
		</cfoutput>
		<cfoutput>
		<tr>
