...
<script>
	var draftremovalnotice=<cfif application.configBean.getPurgeDrafts() and event.getValue("suppressDraftNotice") neq "true" and rc.contentBean.hasDrafts() and not requiresApproval><cfoutput>'#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.draftremovalnotice"))#'</cfoutput><cfelse>""</cfif>;
		siteManager.hasNodeLock=<cfif stats.getLockType() eq 'node'>true<cfelse>false</cfif>;
		<cfoutput>siteManager.unlocknodeconfirm="#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.unlocknodeconfirm'))#";</cfoutput>
</script>
...
<cfif rc.compactDisplay neq "true" and application.configBean.getConfirmSaveAsDraft()>
	...
	function setRequestedURL(){
		...
		return conditionalExit("#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.saveasdraft"))#");
	}</cfoutput>
	...
	function conditionalExit(msg){
		...
		if(msg==null){
			<cfoutput>msg="#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.saveasdraft"))#";</cfoutput>
		}
...
<script>
	<cfif requiresApproval>
		...
		<cfif not approvalRequest.getIsNew() and approvalRequest.getStatus() eq 'Pending'>
			...
			var cancelPendingApproval=<cfoutput>'#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,"approvalchains.cancelPendingApproval"))#'</cfoutput>;
		<cfelse>
...
<cfsilent>
	...
	<div class="form-actions">
		 ...
		 <button type="button" class="btn" onclick="return saveDraftPrompt();"><i class="icon-edit"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savedraft"))#</button>
		 <script>
		 
				saveDraftPrompt=function(){
					confirmDialog(
						'#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.keepeditingconfirm"))#',
						function(){
		...
		<button type="button" class="btn" onclick="document.contentForm.approved.value=0;document.contentForm.preview.value=1;if(siteManager.ckContent(draftremovalnotice)){submitForm(document.contentForm,'add');}"><i class="icon-eye-open"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savedraftandpreview"))#</button>
		</cfif>
		<cfif assignChangesets>
			<button type="button" class="btn" onclick="document.contentForm.approved.value=0;saveToChangeset('#rc.contentBean.getChangesetID()#','#tempEncodeForHTML(rc.siteID)#','');return false;"> 
				<cfif requiresApproval>
					<i class="icon-list"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savetochangesetandsendforapproval"))#
				<cfelse>
					<i class="icon-list"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.savetochangeset"))#
				</cfif>
				</button>	
		</cfif>
		<cfif rc.perm eq 'editor' and not $.siteConfig('EnforceChangesets')>
			<button type="button" class="btn" onclick="document.contentForm.approved.value=1;if(siteManager.ckContent(draftremovalnotice)){submitForm(document.contentForm,'add');}">
				<cfif requiresApproval>
					<i class="icon-share-alt"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.sendforapproval"))#
				<cfelse>
					<i class="icon-check"></i> #tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,"sitemanager.content.publish"))#
				</cfif>
			</button>
		</cfif> 
	</div>
...
<cfoutput>
	...
	<cfif rc.compactDisplay neq "true">
		<ul class="metadata-horizontal">
					...
					<cfif not rc.contentBean.getIsNew()>
						<cfif rc.contentBean.getactive() gt 0 and rc.contentBean.getapproved() gt 0>
							<cfif len(rc.contentBean.getApprovalStatus())>
								<a href="##" onclick="return viewStatusInfo('#tempEncodeForJavascript(rc.contentBean.getContentHistID())#','#tempEncodeForJavascript(rc.contentBean.getSiteID())#');">#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.published")#</a>
							<cfelse>
						...
						<cfelseif len(rc.contentBean.getApprovalStatus()) and requiresApproval >
							<a href="##" onclick="return viewStatusInfo('#tempEncodeForJavascript(rc.contentBean.getContentHistID())#','#tempEncodeForJavascript(rc.contentBean.getSiteID())#');">#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.#rc.contentBean.getApprovalStatus()#")#</a>
			...
			<cfset started=false>
			<li>
				#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.type")#: <strong>#tempEncodeForHTML(rc.type)#</strong>
			</li>
		</ul>
	</cfif>
	...
	<cfif hasChangesets and (not currentChangeset.getIsNew() or pendingChangesets.recordcount)>
		...
		<cfloop query="pendingChangesets"><a href="?muraAction=cArch.edit&moduleID=#tempEncodeForURL(rc.moduleID)#&siteID=#tempEncodeForURL(rc.siteID)#&topID=#tempEncodeForURL(rc.topID)#&contentID=#tempEncodeForURL(rc.contentID)#&return=#tempEncodeForURL(rc.return)#&contentHistID=#pendingChangesets.contentHistID#&parentID=#tempEncodeForURL(rc.parentID)#&startrow=#tempEncodeForURL(rc.startrow)#&type=#tempEncodeForURL(rc.type)#&compactDisplay=#tempEncodeForURL(rc.compactDisplay)#"><strong>#tempEncodeForHTML(pendingChangesets.changesetName)#</strong></a><cfif pendingChangesets.currentrow lt pendingChangesets.recordcount>, </cfif></cfloop><br/></cfif>
		<cfif not currentChangeset.getIsNew()>#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.changesetversionnotify")#: <strong>#tempEncodeForHTML(currentChangeset.getName())#</strong></cfif>
		</p>
	</cfif>

	<cfif len(rc.contentBean.getNotes())>
		<p class="alert">#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.notes")#: #tempEncodeForHTML(rc.contentBean.getNotes())#</p>
	</cfif>
	...
	<cfelse>
		...
		<cfoutput><input type="hidden" name="parentid" value="#tempEncodeForHTMLAttribute(rc.parentid)#"></cfoutput>
	</cfif>
	</cfsavecontent>
	<cfoutput>
	...
	<cfif listFindNoCase("Page,Folder,Calendar,Gallery,Link,File,Component,Form",rc.contentBean.getType())>
		<script type="text/javascript">
		siteManager.tablist='#tempEncodeForJavascript(lcase(tabList))#';
	...
	<input type="hidden" name="return" value="#tempEncodeForHTMLAttribute(rc.return)#">
	...
	<input type="hidden" name="type" value="#tempEncodeForHTMLAttribute(rc.type)#">
	<input type="hidden" name="subtype" value="#rc.contentBean.getSubType()#">
	<input type="hidden" name="muraAction" value="cArch.update">
	<input type="hidden" name="startrow" value="#tempEncodeForHTMLAttribute(rc.startrow)#">
	<input type="hidden" name="returnURL" id="txtReturnURL" value="#rc.returnURL#">
	<input type="hidden" name="homeID" value="#tempEncodeForHTMLAttribute(rc.homeID)#">
	...
	<input type="hidden" name="closeCompactDisplay" value="#tempEncodeForHTMLAttribute(rc.compactDisplay)#" />
	<input type="hidden" name="compactDisplay" value="#tempEncodeForHTMLAttribute(rc.compactDisplay)#" />	
