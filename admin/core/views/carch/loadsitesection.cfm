...
<cf_dsp_nest topid="#tempEncodeForHTMLAttribute(rc.contentID)#" parentid="#tempEncodeForHTMLAttribute(rc.contentID)#"  rsnest="#rsNext#" locking="#application.settingsManager.getSite(rc.siteid).getlocking()#" nestlevel="1" perm="#perm#" siteid="#rc.siteid#" moduleid="#tempEncodeForHTMLAttribute(rc.moduleid)#" restricted="#r#" viewdepth="1" nextn="#session.mura.nextN#" startrow="#tempEncodeForHTMLAttribute(rc.startrow)#" sortBy="#tempEncodeForHTMLAttribute(rc.sortBy)#" sortDirection="#tempEncodeForHTMLAttribute(rc.sortDirection)#" pluginEvent="#pluginEvent#" isSectionRequest="true" muraScope="#rc.$#">
</cfsavecontent>
