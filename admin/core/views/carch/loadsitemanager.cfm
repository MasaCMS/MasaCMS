...
<cfif rc.type neq 'Component' and rc.type neq 'Creative'  and rc.type neq 'Form'>
  ...
  <form novalidate="novalidate" class="viewUpdate clearfix" name="viewUpdate" method="post" action="./index.cfm?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteID)#&moduleid=#tempEncodeForURL(rc.moduleID)#&topid=#tempEncodeForURL(rc.topID)#">
            ...
            <input type="hidden" name="saveSort" value="true">
              <select name="sortBy"  onchange="siteManager.setAsSorted();">
                ...
                <cfloop query="rsExtend">
                  <option value="#tempEncodeForHTMLAttribute(rsExtend.attribute)#" <cfif rc.sortBy eq rsExtend.attribute>selected</cfif>>#rsExtend.Type#/#rsExtend.subType# - #rsExtend.attribute#</option>
                </cfloop>
              </select>
  ...
  <cfif rc.moduleid eq '00000000000000000000000000000000000' and rc.sortBy eq 'orderno'>  
    ...
    <dl class="mura-grid-hdr">
      <dt>
      		<span class="add"></span>
      		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.managerTitle"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.title")#</a>
      </dt>
      <cfif application.settingsManager.getSite(rc.siteid).getlocking() neq 'all'>
        <dd class="objects"><a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.managerObjects"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.objects")#</a></dd>
        <dd class="display"><a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.managerDisplay"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.display")#</a></dd>
        <dd class="template"><a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.managerTemplate"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.template")#</a></dd>
      </cfif>
      <dd class="nav"><a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.managerNav"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.nav")#</a></dd>
      <dd class="updated"><a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.managerUpdated"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.updated")#</a></dd>
      <dd class="actions">&nbsp;</dd>
    </dl>
    <ul id="mura-nodes"<cfif arrayLen(crumbdata) gt 1 and crumbdata[2].type eq 'Gallery'> class="gallery"</cfif>>
    <!-- Begin List of Nodes -->
    <li data-siteid="#tempEncodeForHTMLAttribute(rc.siteid)#" data-contentid="#rc.rstop.contentid#" data-contenthistid="#rc.rstop.contenthistid#" data-moduleid="#tempEncodeForHTMLAttribute(rc.moduleid)#" data-sortby="#tempEncodeForHTMLAttribute(rc.rstop.sortby)#" data-sortdirection="#tempEncodeForHTMLAttribute(rc.rstop.sortdirection)#" class="#tempEncodeForHTMLAttribute(lcase(rc.rstop.type))# mura-node-data<cfif r> restricted</cfif>" data-csrf="#tempEncodeForHTMLAttribute(rc.$.renderCSRFTOkens(context=rc.rstop.contentid & 'quickedit',format='url'))#">
     <cfif r><div class="marker"></div></cfif>
      ...
      <dt>
       ...
       <a class="add" href="javascript:;" onmouseover="siteManager.showMenu('newContentMenu','#newcontent#',this,'#rc.rstop.contentid#','#rc.topid#','#rc.rstop.parentid#','#rc.siteid#','#rc.rstop.type#');"><i class="icon-plus-sign"></i></a>
        ...
        <cfif hasKids>
	    	<span class="hasChildren open" onclick="siteManager.loadSiteManager('#tempEncodeForJavascript(rc.siteID)#','#tempEncodeForJavascript(rc.topid)#','#tempEncodeForJavascript(rc.moduleid)#','#tempEncodeForJavascript(rc.sortby)#','#tempEncodeForJavascript(rc.sortdirection)#','#tempEncodeForJavascript(rc.rstop.type)#',1);"></span>
		</cfif>
      ...
      <cfelse>
        ...
        <cfif not listFindNoCase('none,read',perm)>
          <a class="<cfif isFileIcon>file #lcase(icon)#<cfelse>#lcase(icon)#</cfif> title draftprompt" title="#atitle#" href="./?muraAction=cArch.edit&contenthistid=#rc.rstop.ContentHistID#&siteid=#tempEncodeForURL(rc.siteid)#&contentid=#rc.topid#&topid=#tempEncodeForURL(rc.topid)#&type=#rc.rstop.type#&parentid=#rc.rstop.parentid#&moduleid=#rc.moduleid#"<cfif rc.rsTop.type eq 'File'> data-filetype="#lcase(left(rc.rsTop.fileExt,4))#"</cfif> <cfif atooltip>rel="tooltip" data-html="true"</cfif>>
        <cfelse>
		...
		#tempEncodeForHTML(left(rc.rsTop.menutitle,75))#
     ...
     <cfif application.settingsManager.getSite(rc.siteid).getlocking() neq 'all'>
          ...
          <a class="mura-quickEditItem<cfif rc.rstop.Display eq 2 and rc.rstop.approved> tooltip</cfif>" data-attribute="display"></cfif>
            ...
            <cfelseif rc.rstop.Display eq 2 and rc.rstop.approved>
           	 	<cfif not (perm eq 'editor' and request.hasPublishingTab)>
           	 		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute('#LSDateFormat(rc.rstop.displaystart,"short")#&nbsp;-&nbsp;#LSDateFormat(rc.rstop.displaystop,"short")#')#"></a>
           	 	 </cfif>
      ...
      <dd class="actions">
          ...
          <cfif not listFindNoCase('none,read',perm)>
            ...
            <li class="edit<cfif isLockedBySomeoneElse> disabled</cfif>"><a class="draftprompt" data-siteid="#rc.siteid#" data-contentid="#rc.rstop.contentid#" data-contenthistid="#rc.rstop.contenthistid#" title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.edit")#" href="./?muraAction=cArch.edit&contenthistid=#rc.rstop.ContentHistID#&siteid=#tempEncodeForURL(rc.siteid)#&contentid=#rc.topid#&topid=#tempEncodeForURL(rc.topid)#&type=#rc.rstop.type#&parentid=#rc.rstop.parentid#&moduleid=#rc.moduleid#"><i class="icon-pencil"></i></a></li>
            ...
            <cfif listFind(session.mura.memberships,'Admin;#application.settingsManager.getSite(rc.siteid).getPrivateUserPoolID()#;0') or listFind(session.mura.memberships,'S2')>
              <li class="permissions"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.permissions")#" href="./?muraAction=cPerm.main&contentid=#rc.topid#&parentid=&topid=#tempEncodeForURL(rc.topid)#&siteid=#tempEncodeForURL(rc.siteid)#&moduleid=#rc.moduleid#&type=#rc.rstop.type#"><i class="icon-group"></i></a></li>
              <cfelse>
