  ...
  <cfif rc.moduleid eq '00000000000000000000000000000000000' and rc.sortBy eq 'orderno'>  
    ...
    <dl class="mura-grid-hdr">
      ...
      <cfif application.settingsManager.getSite(rc.siteid).getlocking() neq 'all'>
        ...
        <dd class="template"><a href="##"<cfif len(rc.rstop.template) or len(rc.rstop.template)> class="template-set"</cfif> rel="tooltip" title="#HTMLEditFormat(application.rbFactory.getKeyValue(session.rb,"tooltip.managerTemplate"))#">#application.rbFactory.getKeyValue(session.rb,"sitemanager.template")#</a></dd>
      </cfif>
