...
<cfif $.siteConfig('hasLockableNodes')>
	...
	<cfif nodeLockedByYou or nodeLockedBySomeElse>
		...
		<cfelse>
			...
			<p id="msg-node-locked-else" class="alert alert-error">#application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"sitemanager.nodeLockedby"),"#tempEncodeForHTMLAttribute(lockedBy.getFName())# #tempEncodeForHTML(lockedBy.getLName())#")#.<br>
			<a href="mailto:#tempEncodeForHTML(lockedBy.getEmail())#?subject=#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,'sitemanager.nodeunlockrequest'))#"><i class="icon-envelope"></i> #application.rbFactory.getKeyValue(session.rb,'sitemanager.requestnoderelease')#</a>
			...
			$(function(){
				jQuery(".mura-node-unlock").click(
					function(event){
						...
						confirmDialog(
							"#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.unlocknodeconfirm'))#",
							function(){
...
<cfif requiresApproval and listFindNoCase('Pending,Rejected',rc.contentBean.getApprovalStatus())  >
	<p class="alert alert-error">
		...
		<strong><a href="##" onclick="return viewStatusInfo('#tempEncodeForJavascript(rc.contentBean.getContentHistID())#','#tempEncodeForJavascript(rc.contentBean.getSiteID())#');">#application.rbFactory.getKeyValue(session.rb,"sitemanager.content.viewdetails")#</a></strong>
	</p>
</cfif>
<script>
...
function applyApprovalAction(requestid,action,comment,siteid){
	...
	if(action == 'Reject' && comment == ''){
		alertDialog('#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,"approvalchains.rejectioncommentrequired"))#');
	} else {
...
<div style="display:none;" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"layout.status"))#" id="approvalModalContainer">
