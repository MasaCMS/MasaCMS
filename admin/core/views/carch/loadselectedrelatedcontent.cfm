...
<cfoutput>
	...
	<div id="mura-rc-quickedit" style="display:none;">
		...
		<cfloop from="1" to="#arrayLen(relatedContentSets)#" index="s">
			...
			<li><input id="mura-rc-option-label#s#" type="checkbox" class="mura-rc-quickassign" value="#rcsBean.getRelatedContentSetID()#"/> <label for="mura-rc-option-label#s#">#tempEncodeForHTML(rcsBean.getName())#</label></li>
		</cfloop>
		</ul>
	</div>
	...
	<cfloop from="1" to="#arrayLen(relatedContentSets)#" index="s">
		...
		<cfoutput>
			<div id="rcGroup-#rcsBean.getRelatedContentSetID()#" class="list-table">
				<div class="list-table-content-set">#tempEncodeForHTML(rcsBean.getName())# 
