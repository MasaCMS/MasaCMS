...
<cfoutput>
	<cfif rc.classid eq "related_content">
		...
		data-name="#encodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.relatedcontent'))#" 
		data-objectid="#createUUID()#">
	<cfelse>
		<cfset menutitle=$.getBean("content").loadBy(contentID=rc.contentID).getMenuTitle()>
		<div id="availableObjectParams"	
		data-object="#rc.classid#" 
		data-name="#encodeForHTMLAttribute('#menutitle# - #application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.relatedcontent')#')#" 
		data-objectid="#hash('related_content')#">
	</cfif>
	
<!---<cfif rc.classid eq "related_content">
<h2>#encodeForHTML(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.relatedcontent'))#</h2>
	<cfelse>
		<h2>#encodeForHTML('#menutitle# - #application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.relatedcontent')#')#</h2>
	</cfif>
--->
			...
			<div class="span6">
				...
				<div class="controls">
					<select name="sortBy" class="span12 objectParam">
						...
						<cfloop query="rsExtend"><option value="#encodeForHTMLAttribute(rsExtend.attribute)#" <cfif feed.getsortBy() eq rsExtend.attribute>selected</cfif>>#rsExtend.Type#/#rsExtend.subType# - #rsExtend.attribute#</option>
						</cfloop>
					--->
					</select>
				</div>
