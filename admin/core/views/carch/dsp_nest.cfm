...
<li data-siteid="#tempEncodeForHTMLAttribute(rsnest.siteid)#" data-contentid="#rsnest.contentid#" data-contenthistid="#rsnest.contenthistid#" data-sortby="#tempEncodeForHTMLAttribute(rsnest.sortby)#" data-sortdirection="#tempEncodeForHTMLAttribute(rsnest.sortdirection)#" data-moduleid="#tempEncodeForHTMLAttribute(attributes.moduleid)#" data-type="#tempEncodeForHTMLAttribute(rsnest.type)#" class="#tempEncodeForHTMLAttribute(lcase(rsnest.type))# mura-node-data<cfif variables.restricted> restricted</cfif>" data-csrf="#tempEncodeForHTMLAttribute(attributes.muraScope.renderCSRFTOkens(context=rsnest.contentid & 'quickedit',format='url'))#">
...
<dt>
	...
	<cfif not listFindNoCase('none,read',verdict)>
		<a class="<cfif isFileIcon>file #lcase(icon)#<cfelse>#lcase(icon)#</cfif> title draftprompt" title="#atitle#" href="./?muraAction=cArch.edit&contenthistid=#rsnest.ContentHistID#&contentid=#rsnest.ContentID#&type=#rsnest.type#&parentid=#rsnest.parentID#&topid=#tempEncodeForURL(attributes.topid)#&siteid=#tempEncodeForURL(attributes.siteid)#&moduleid=#attributes.moduleid#&startrow=#attributes.startrow#"<cfif rsnest.type eq 'File'> data-filetype="#lcase(left(rsnest.fileExt,4))#"</cfif>  <cfif atooltip>rel="tooltip" data-html="true"</cfif>>
	<cfelse>
	...
	#tempEncodeForHTML(left(rsnest.menutitle,75))#
	...
	<dd class="display<cfif rsnest.Display eq 2 and rsnest.approved> scheduled</cfif>">
		...
		<cfelseif rsnest.Display eq 2 and rsnest.approved>
			<cfif not (verdict eq 'editor' and request.hasPublishingTab)>
				<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(LSDateFormat(rsnest.displaystart,"short"))#&nbsp;-&nbsp;#LSDateFormat(rsnest.displaystop,"short")#">
			</cfif>
    ...
    <ul>
	   ...
	   <li class="version-history"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.versionhistory")#" href="./?muraAction=cArch.hist&contentid=#rsnest.ContentID#&type=#rsnest.type#&parentid=#rsnest.parentID#&topid=#tempEncodeForURL(attributes.topid)#&siteid=#tempEncodeForURL(attributes.siteid)#&moduleid=#attributes.moduleid#&startrow=#attributes.startrow#"><i class="icon-book"></i></a></li>
        <cfif listFind(session.mura.memberships,'Admin;#application.settingsManager.getSite(attributes.siteid).getPrivateUserPoolID()#;0') or listFind(session.mura.memberships,'S2')>
          <li class="permissions"><a title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.permissions")#" href="./?muraAction=cPerm.main&contentid=#rsnest.ContentID#&type=#rsnest.type#&parentid=#rsnest.parentID#&topid=#tempEncodeForURL(attributes.topid)#&siteid=#tempEncodeForURL(attributes.siteid)#&moduleid=#attributes.moduleid#&startrow=#attributes.startrow#"><i class="icon-group"></i></a></li>
        <cfelse>
		  <li class="permissions disabled"><a><i class="icon-group"></i></a></li>
		</cfif>
        <cfif deletable and not isLockedBySomeoneElse>
          <li class="delete"><a  title="#application.rbFactory.getKeyValue(session.rb,"sitemanager.delete")#" href="./?muraAction=cArch.update&contentid=#rsnest.ContentID#&type=#rsnest.type#&action=deleteall&topid=#tempEncodeForURL(attributes.topid)#&siteid=#tempEncodeForURL(attributes.siteid)#&moduleid=#attributes.moduleid#&parentid=#tempEncodeForURL(attributes.parentid)#&startrow=#attributes.startrow##attributes.muraScope.renderCSRFTokens(context=rsnest.contentid & 'deleteall',format='url')#" onclick="return confirmDialog('#tempEncodeForJavascript(application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.deletecontentrecursiveconfirm'),rsnest.menutitle))#',this.href)"><i class="icon-remove-sign"></i></a></li>
