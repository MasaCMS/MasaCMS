...
<dt>
	...
	<cfsilent>
		<cfif verdict neq 'none' and listFindNoCase("jpg,jpeg,png,gif",listLast(rsnest.assocfilename,"."))>
			<cfset atooltip=true>
			<cfset atitle="<img class='image-preview' src='#$.getURLForImage(fileid=rsnest.fileid,size='small',siteid=rsnest.siteid,fileext=rsnest.fileExt,useProtocol=false)#'>">
		<cfelse>
...
<cfelse>
    ...
    <dd class="updated" title="#LSDateFormat(rsnest.lastupdate,session.dateKeyFormat)# #LSTimeFormat(rsnest.lastupdate,"medium")#">#LSDateFormat(rsnest.lastupdate,session.dateKeyFormat)#</dd>
    <dd class="actions">
    <ul>
    	<cfif not listFindNoCase('none,read',verdict)>
