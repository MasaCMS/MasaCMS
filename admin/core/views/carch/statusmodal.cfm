...
<div id="status-modal" class="mura-list-grid">
	<dl>
		...
		<dd>
			<i class="icon-user"></i>
			<p><cfif not user.getIsNew()>#tempEncodeForHTML(user.getFullName())# <cfelse> #application.rbFactory.getKeyValue(session.rb,"sitemanager.content.na")# </cfif></p>
		</dd>
	</dl>
	...
	<dl>
		...
		<dl class="change-set">
			...
			<dd>
				<i class="icon-list"></i>
				<p><cfif changeset.getIsNew()>Unassigned<cfelse>#tempEncodeForHTML(changeset.getName())#</cfif></p>
			</dd>
		</dl>
		</cfif>
		...
		<cfif requiresApproval>
			...
			<dl class="approval-chain-comments">
				...
				<dd>
					...
					<cfif len(action.getComments())><p><!--- <i class="icon-comment"></i>  --->#tempEncodeForHTML(action.getComments())#</p></cfif>
					<em>#application.rbFactory.getKeyValue(session.rb,"approvalchains.by")# #tempEncodeForHTML(action.getUser().getFullName())# #application.rbFactory.getKeyValue(session.rb,"approvalchains.on")# #LSDateFormat(parseDateTime(action.getCreated()),session.dateKeyFormat)# #application.rbFactory.getKeyValue(session.rb,"approvalchains.at")# #LSTimeFormat(parseDateTime(action.getCreated()),"short")#</em>		
				</dd>
			...
			<cfif not content.getApproved() and approvalRequest.getStatus() eq 'Pending' and (isApprover or isRequester)>
				...
				<input id="mura-approval-apply" type="button" class="btn btn-primary" value="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.apply'))#" onclick="applyApprovalAction('#approvalRequest.getRequestID()#',$('input:radio[name=approval-action]:checked').val(),$('##approval-comments').val(),'#approvalRequest.getSiteID()#');"/>
				</dd>
				</dl>
			</cfif>
		</cfif>
