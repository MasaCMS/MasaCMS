	...
	<cfelseif rc.attribute eq "display">
		...
		<ol id="mura-quickEdit-displayDates"<cfif content.getdisplay() NEQ 2> style="display: none;"</cfif>>
			...
			<input type="text" id="mura-quickEdit-displayStart" value="#LSDateFormat(content.getdisplaystart(),session.dateKeyFormat)#" class="textAlt datepicker mura-quickEdit-datepicker"><br />
				<select id="mura-quickEdit-startHour" class="dropdown span1"><cfloop from="1" to="12" index="h"><option value="#h#" <cfif not LSisDate(content.getdisplaystart())  and h eq 12 or (LSisDate(content.getdisplaystart()) and (hour(content.getdisplaystart()) eq h or (hour(content.getdisplaystart()) - 12) eq h or hour(content.getdisplaystart()) eq 0 and h eq 12))>selected</cfif>>#h#</option></cfloop></select>
				<select id="mura-quickEdit-startMinute" class="dropdown span1"><cfloop from="0" to="59" index="m"><option value="#m#" <cfif LSisDate(content.getdisplaystart()) and minute(content.getdisplaystart()) eq m>selected</cfif>>#iif(len(m) eq 1,de('0#m#'),de('#m#'))#</option></cfloop></select>
				<select id="mura-quickEdit-startDayPart" class="dropdown span1"><option value="AM">AM</option><option value="PM" <cfif LSisDate(content.getdisplaystart()) and hour(content.getdisplaystart()) gte 12>selected</cfif>>PM</option></select></li>
			...
			<input type="text" id="mura-quickEdit-displayStop" value="#LSDateFormat(content.getdisplaystop(),session.dateKeyFormat)#" class="textAlt datepicker mura-quickEdit-datepicker"><br />
				<select id="mura-quickEdit-stopHour" class="dropdown span1"><cfloop from="1" to="12" index="h"><option value="#h#" <cfif not LSisDate(content.getdisplaystop())  and h eq 11 or (LSisDate(content.getdisplaystop()) and (hour(content.getdisplaystop()) eq h or (hour(content.getdisplaystop()) - 12) eq h or hour(content.getdisplaystop()) eq 0 and h eq 12))>selected</cfif>>#h#</option></cfloop></select>
