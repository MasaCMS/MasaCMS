...
<option value="#tempEncodeForHTMLAttribute(f)#" <cfif f eq rc.sortBy>selected</cfif>>#tempEncodeForHTML(f)#</option>
</cfloop>
</select>
...
<div class="control-group">
	...
	<div class="controls">
		...
		<cfloop list="#rc.fieldnames#" index="f">
		<option value="#tempEncodeForHTMLAttribute(f)#" <cfif f eq session.filterBy>selected</cfif>>#tempEncodeForHTML(f)#</option>
		</cfloop>
		</select>
		<input type="text" class="span6" name="keywords" value="#tempEncodeForHTMLAttribute(session.datakeywords)#">
	</div>
</div>
</div>
...
<input type="hidden" name="siteid" value="#tempEncodeForHTMLAttribute(session.siteid)#" />
...
<div class="form-actions">
	...
	<button type="button" class="btn" onclick="location.href='./?muraAction=cArch.downloaddata&siteid=#tempEncodeForURL(rc.siteid)#&contentid=#tempEncodeForURL(rc.contentid)#&date1=' + document.download.date1.value + '&hour1=' +document.download.hour1.value + '&minute1=' +document.download.minute1.value + '&date2=' + document.download.date2.value + '&hour2=' + document.download.hour2.value + '&minute2=' + document.download.minute2.value + '&sortBy=' +  document.download.sortBy.value + '&sortDirection=' +  document.download.sortDirection.value + '&filterBy='  + document.download.filterBy.value + '&keywords=' + document.download.keywords.value + '&columns=#tempEncodeForURL(rc.columns)#';"><i class="icon-download"></i> #application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.download')#</button>
</div>
</form></cfoutput>
</cfif>
...
<cfloop list="#rc.fieldnames#" index="f">
<th><cfoutput>#tempEncodeForHTML(f)#</cfoutput></th>
</cfloop>
</tr>
</thead>
<tbody>
...
<cftry>
	...
	<td class="actions">
		<ul>
			<li class="edit">
				<a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.edit')#" href="./?muraAction=cArch.datamanager&contentid=#tempEncodeForURL(rc.contentid)#&siteid=#tempEncodeForURL(rc.siteid)#&date1=#tempEncodeForURL(rc.date1)#&hour1=#tempEncodeForURL(rc.hour1)#&minute1=#tempEncodeForURL(rc.minute1)#&date2=#tempEncodeForURL(rc.date2)#&hour2=#tempEncodeForURL(rc.hour2)#&minute2=#tempEncodeForURL(rc.minute2)#&responseid=#rsdata.responseid#&action=edit&moduleid=#tempEncodeForURL(rc.moduleid)#&sortBy=#tempEncodeForURL(rc.sortBy)#&sortDirection=#tempEncodeForURL(rc.sortDirection)#&filterBy=#tempEncodeForURL(rc.filterBy)#&keywords=#tempEncodeForURL(rc.keywords)#"><i class="icon-pencil"></i></a>
			</li>
			<li class="delete">
				<a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.deleteresponse')#" href="./?muraAction=cArch.datamanager&contentid=#tempEncodeForURL(rc.contentid)#&siteid=#tempEncodeForURL(rc.siteid)#&date1=#tempEncodeForURL(rc.date1)#&hour1=#tempEncodeForURL(rc.hour1)#&minute1=#tempEncodeForURL(rc.minute1)#&date2=#tempEncodeForURL(rc.date2)#&hour2=#tempEncodeForURL(rc.hour2)#&minute2=#tempEncodeForURL(rc.minute2)#&responseid=#rsdata.responseid#&action=delete&moduleid=#tempEncodeForURL(rc.moduleid)#&sortBy=#tempEncodeForURL(rc.sortBy)#&sortDirection=#tempEncodeForURL(rc.sortDirection)#&filterBy=#tempEncodeForURL(rc.filterBy)#&keywords=#tempEncodeForURL(rc.keywords)##rc.$.renderCSRFTokens(context=rsdata.responseID,format='url')#" onclick="return confirmDialog('#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.deleteresponseconfirm'))#',this.href)"><i class="icon-remove-sign"></i></a>
			</li>
		</ul>
	</td>
	...
	<td class="mForm-data"><cfif findNoCase('attachment',f) and isValid("UUID",fvalue)><a  href="##" onclick="return preview('http://#application.settingsManager.getSite(rc.siteid).getDomain()##application.configBean.getServerPort()##application.configBean.getContext()#/tasks/render/file/?fileID=#tempEncodeForURL(fvalue)#');">#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.viewattachment')#</a><cfelse>#$.setParagraphs(tempEncodeForHTML(fvalue))#</cfif></td>
	</cfloop>
	<cfcatch>
