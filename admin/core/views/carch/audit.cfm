...
<cfif hasChangesets>
	<td class="changeset">
		...
		<cfif isDate(changeset.getPublishDate())><a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(LSDateFormat(changeset.getPublishDate(),"short"))#"> <i class="icon-calendar"></i></a></cfif>
		<cfif hasChangesetAccess>
			<a href="./?muraAction=cChangesets.assignments&siteID=#rc.item.getsiteid()#&changesetID=#rc.item.getchangesetID()#">		#tempEncodeForHTML(changeset.getName())#
			</a>
		<cfelse>
			#tempEncodeForHTML(changeset.getName())#
		</cfif>
		</cfif>
	</td>
</cfif> 
