					...
					<dt class="categorytitle">
						...
						<label>
							<cfif rslist.isOpen eq 1><input name="categoryid"<cfif attributes.disabled>
								disabled="true"
							</cfif> value="#rslist.categoryid#" type="checkbox" <cfif rsIsMember.recordcount>	checked="true"</cfif>/> </cfif>#tempEncodeForHTML(rslist.name)#</label>
						...
						<cfif rslist.isOpen eq 1 and rslist.isFeatureable eq 1 or rslist.isFeatureable eq ''>
							<div id="categoryLabelContainer#cattrim#" class="categoryLabelContainer">
								<div class="categoryassignment<cfif rsIsMember.recordcount and rsIsMember.isFeature eq 2> scheduled</cfif>">
									<!--- Quick Edit --->
									<a class="dropdown-toggle<cfif not attributes.disabled> mura-quickEditItem</cfif>"<cfif rsIsMember.isFeature eq 2> rel="tooltip" title="#tempEncodeForHTMLAttribute(LSDateFormat(rsIsMember.featurestart,"short"))#&nbsp;-&nbsp;#LSDateFormat(rsIsMember.featurestop,"short")#"</cfif>>
										<cfswitch expression="#rsIsMember.isFeature#">
											...
											<cfcase value="2">
												<i class="icon-calendar" title="#tempEncodeForHTMLAttribute(LSDateFormat(rsIsMember.featurestart,"short"))#&nbsp;-&nbsp;#LSDateFormat(rsIsMember.featurestop,"short")#"></i>
											</cfcase>
											<cfdefaultcase>
