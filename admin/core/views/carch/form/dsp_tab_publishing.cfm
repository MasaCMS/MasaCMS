	...
	<div class="fieldset">
  	...
  	<cfif listFindNoCase('Page,Folder,Calendar,Gallery,File,Link',rc.type)>
		<div class="control-group">
			      ...
			      <div class="controls"><input type="text" id="credits" name="credits" value="#tempEncodeForHTMLAttribute(rc.contentBean.getCredits())#"  maxlength="255" class="span12"></div>
  		...
  		<div class="control-group">
		     <div class="controls">
		      	<label for="isNav" class="checkbox">
		      		...
		      		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.includeSiteNav"))#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.isnav')#
		...
		<div class="control-group">
		    <div class="controls">
		     	...
		     	<input  name="target" id="Target" type="CHECKBOX" value="_blank" <cfif rc.contentBean.gettarget() eq "_blank">checked</cfif> class="checkbox" > 
		     		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.openNewWindow"))#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.newwindow')#
		     		 <i class="icon-question-sign"></i></a>
		     	</label>
		     </div>  
		</div> <!--- /end control-group --->
		...
		<div class="control-group">
	      	<label class="control-label">
	      		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.contentReleaseDate"))#">
	...
	<cfif ((rc.parentid neq '00000000000000000000000000000000001' and application.settingsManager.getSite(rc.siteid).getlocking() neq 'all') or (rc.parentid eq '00000000000000000000000000000000001' and application.settingsManager.getSite(rc.siteid).getlocking() eq 'none')) and rc.contentid neq '00000000000000000000000000000000001'>
		...
		<cfif rc.type neq 'Component' and application.settingsManager.getSite(rc.siteid).getlocking() neq 'all' and rc.type neq 'Form'>
			<div class="control-group">
	      		...
	      		<div class="controls" id="mover2" style="display:none"><input type="hidden" name="parentid" value="#tempEncodeForHTMLAttribute(rc.parentid)#"></div>	
				<script>
					jQuery(document).ready(function(){
						$('##selectParent').click(function(e){
							e.preventDefault();
							siteManager.loadSiteParents(
								'#tempEncodeForJavascript(rc.siteid)#'
								,'#tempEncodeForJavascript(rc.contentid)#'
								,'#tempEncodeForJavascript(rc.parentid)#'
								,''
								,1
							);
							return false;
		...
		<div class="control-group">
			...
			<div class="controls" id="expires-notify">
				<label for="dspexpiresnotify" class="checkbox">
					<input type="checkbox" name="dspExpiresNotify" id="dspexpiresnotify" onclick="siteManager.loadExpiresNotify('#tempEncodeForJavascript(rc.siteid)#','#tempEncodeForJavascript(rc.contenthistid)#','#tempEncodeForJavascript(rc.parentid)#');"  class="checkbox">
		...
		<div class="controls">
			<label class="control-label">#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.notifyforreviewlabel')#</label>
	   		<label for="dspnotify" class="checkbox">
	      		<input type="checkbox" name="dspNotify"  id="dspnotify" onclick="siteManager.loadNotify('#tempEncodeForJavascript(rc.siteid)#','#tempEncodeForJavascript(rc.contentid)#','#tempEncodeForJavascript(rc.parentid)#');"  class="checkbox"> 
	      		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.notifyReview"))#">
