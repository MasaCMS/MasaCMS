...
<div id="tabBasic" class="tab-pane">
	...
	<cfswitch expression="#rc.type#">
		<cfcase value="Page,Folder,Calendar,Gallery,File,Link">
			<div class="control-group">
			    <label class="control-label">
			    	<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.pageTitle"))#">
			    ...
			    <div class="controls">
			     	<input type="text" id="title" name="title" value="#tempEncodeForHTMLAttribute(rc.contentBean.gettitle())#"  maxlength="255" class="span12" required="true" message="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.titlerequired')#" <cfif hasSEOTab and not rc.contentBean.getIsNew()>onkeypress="openDisplay('editAdditionalTitles','#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.close')#');"</cfif>>
			     </div>
		...
		<cfdefaultcase>
			...
			<div class="control-group">
	     		 ...
	     		 <div class="controls">
	     		 	<input type="text" id="title" name="title" value="#tempEncodeForHTMLAttribute(rc.contentBean.getTitle())#"  maxlength="255" class="span12" required="true" message="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.titlerequired')#"<cfif rc.contentBean.getIsLocked()> disabled="disabled"</cfif>>
	...
	<cfif not ListFindNoCase('Form,Component',rc.type) >	
		<div class="control-group summaryContainer" style="display:none;">
	      	<label class="control-label">
	      		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,"tooltip.contentSummary"))#">
	      	...
	      	<div id="editSummary" class="controls summaryContainer" style="display:none;">
				<cfoutput>
					<textarea name="summary" id="summary" cols="96" rows="10"><cfif application.configBean.getValue("htmlEditorType") neq "none" or len(rc.contentBean.getSummary())>#tempEncodeForHTML(rc.contentBean.getSummary())#<cfelse><p></p></cfif></textarea>
				</cfoutput>
			</div>
			...
			<cfelse>
				<cfif application.configBean.getValue("htmlEditorType") eq "fckeditor">
					<cfscript>
						...
						{
						fckEditor.config["CustomConfigurationsPath"]=tempEncodeForURL('#application.settingsManager.getSite(rc.siteid).getThemeAssetPath()#/js/fckconfig.js.cfm?EditorType=#rc.type#');
						}
				...
				<cfelseif application.configBean.getValue("htmlEditorType") eq "none">
					<textarea name="body" id="body">#tempEncodeForHTML(rc.contentBean.getBody())#</textarea>
				<cfelse>	
					<textarea name="body" id="body"><cfif len(rc.contentBean.getBody())>#tempEncodeForHTML(rc.contentBean.getBody())#<cfelse><p></p></cfif></textarea>
					<script type="text/javascript">
					var loadEditorCount = 0;
		...
		<div class="control-group">
	     	...
	     	<div class="controls">
	     	 	...
	     	 	<cfif len(application.serviceFactory.getBean('settingsManager').getSite(session.siteid).getRazunaSettings().getHostname())>
	     	 			<input type="text" id="url" name="body" value="#tempEncodeForHTMLAttribute(rc.contentBean.getbody())#" class="text span9" required="true" message="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.urlrequired')#">
	     	 			<div class="btn-group">
	...
	<cfif rc.type eq 'Form'>
		...
		<div class="control-group body-container" style="display:none">
			...
			<div class="controls">
				<textarea name="responseMessage" rows="6" class="span12">#tempEncodeForHTML(rc.contentBean.getresponseMessage())#</textarea>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">
			...
			<div class="controls">
				<input type="text" name="responseSendTo" value="#tempEncodeForHTMLAttribute(rc.contentBean.getresponseSendTo())#" class="span12">
			</div>
		</div> 
	</cfif>
