...
<div id="assocFileContainer" class="control-group" style="display:none">
   <label class="control-label">
	<cfif rc.ptype eq 'Gallery' or rc.type neq 'File'>
		...
		<a href="##" rel="tooltip" title="#tempEncodeForHTMLAttribute(application.rbFactory.getKeyValue(session.rb,'tooltip.selectimage'))#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.selectimage')# <i class="icon-question-sign"></i></a>
	<cfelse>
	...
	<cfif not fileLockedBySomeElse>
		...
		<cfif rc.type eq 'File'>										
			<input type="hidden" name="fileid" value="#tempEncodeForHTMLAttribute(rc.contentBean.getFileID())#" />
		</cfif>
	<cfelse>
		...
		<p id="msg-file-locked-else" class="alert alert-error">#application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"sitemanager.filelockedby"),"#tempEncodeForHTMLAttribute(lockedBy.getFName())# #tempEncodeForHTML(lockedBy.getLName())#")#.<br>
		<a href="mailto:#tempEncodeForHTML(lockedBy.getEmail())#?subject=#tempEncodeForHTML(application.rbFactory.getKeyValue(session.rb,'sitemanager.fileunlockrequest'))#"><i class="icon-envelope"></i> #application.rbFactory.getKeyValue(session.rb,'sitemanager.requestfilerelease')#</a>
	...
	<script>
		...
		siteManager.unlockfileconfirm="#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.fields.unlockfileconfirm'))#";
	</script>
