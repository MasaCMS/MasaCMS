...
<h2>#application.rbFactory.getKeyValue(session.rb,"sitemanager.reports.#$.event('report')#")#</h2>	
	...
	<table class="mura-table-grid">
		...
		<cfset editLink="./?muraAction=cArch.edit&contenthistid=#item.getContentHistID()#&contentid=#item.getContentID()#&type=#item.gettype()#&parentid=#item.getParentID()#&topid=#tempEncodeForURL(topID)#&siteid=#tempEncodeForURL(item.getSiteid())#&moduleid=#item.getmoduleid()#&startrow=#$.event('startrow')#">
		...
		<tr data-siteid="#item.getSiteID()#" data-contentid="#item.getContentID()#" data-contenthistid="#item.getContentHistID()#" data-sortby="#item.getSortBy()#" data-sortdirection="#item.getSortDirection()#" data-moduleid="#tempEncodeForHTMLAttribute(item.getModuleID())#" data-type="#item.getType()#" class="mura-node-data">
				...
				<ul class="siteSummary">
					<cfif not listFindNoCase('none,read',verdict) or $.event('report') eq 'mydrafts'>
						...
						<cfswitch expression="#item.gettype()#">
							...
							<li class="download"><a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.download')#" href="/tasks/render/file/?fileID=#item.getFileID()#&method=attachment" onclick="return confirmDialog('#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'sitemanager.downloadconfirm'))#',this.href)"><i class="icon-download"></i></a></li>
							</cfif>
							</cfcase>
						</cfswitch>
						 
						 <li class="version-history"><a title="Version History" href="./?muraAction=cArch.hist&contentid=#item.getContentID()#&type=#item.gettype()#&parentid=#item.getparentID()#&topid=#topid#&siteid=#tempEncodeForURL(item.getSiteID())#&moduleid=#item.getmoduleid()#&startrow=#$.event('startrow')#"><i class="icon-book"></i></a></li>
			...
			<h2>
				...
				<cfelse>
					#tempEncodeForHTML(item.getMenuTitle())#
					<!---
					<cfif $.event('report') eq 'mydrafts'>
						(
						<cfif rsHasPendingApprovals.recordcount>
			...
			<div class="thumbnail"><a title="Edit" class="draftprompt" href="#editLink#"><cfif hasCustomImage><img src="#item.getImageURL(height=80,width=80)#" /><cfelse><img src="#item.getImageURL(size='small')#" /></cfif></a></div>
				...
				<cfif len(item.getLockID())>
					...
					<cfif item.getLockType() neq 'node'>
						<p class="locked-offline"><i class="icon-lock"></i> #application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"sitemanager.filelockedby"),"#tempEncodeForHTMLAttribute(lockedBy.getFName())# #tempEncodeForHTML(lockedBy.getLName())#")#</p>
					<cfelseif $.siteConfig('hasLockableNodes')>
						<p class="locked-offline"><i class="icon-lock"></i> #application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"sitemanager.nodelockedby"),"#tempEncodeForHTMLAttribute(lockedBy.getFName())# #tempEncodeForHTML(lockedBy.getLName())#")#</p>
					</cfif>
				...
				<ul class="nodeMeta">
					...
					<li class="categories">#application.rbFactory.getKeyValue(session.rb,"sitemanager.categories")#: <strong>
						<cfloop condition="categories.hasNext()">
							...
							#tempEncodeForHTML(category.getName())#<cfif categories.hasNext()>, </cfif>
						</cfloop>
						</strong>
					</cfif></dd>
...
<div class="sidebar span3">
    ...
    <input class="text" id="contentKeywords" value="#tempEncodeForHTMLAttribute(session.flatViewArgs["#rc.siteID#"].keywords)#" type="text" size="20" />
  	</div>
...
<script>
	function clearFlatviewFilter(){
		flatViewArgs=$.extend(
			initFlatViewArgs(),
			{
					report:'#tempEncodeForJavascript(session.flatViewArgs["#rc.siteID#"].report)#',
