...
<div class="row-fluid">
	  ...
	  <table class="mura-table-grid">
		...
		<thead>
		<tr> 
	      <th class="var-width"><a href="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&topid=#tempEncodeForURL(rc.topid)#&parentid=#tempEncodeForURL(rc.parentid)#&moduleid=#rc.moduleID#&sortBy=title&sortDirection=#titleDirection#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.title')#</a></th>
	    <!--- <cfif rc.perm eq 'editor'><th class="order" width="30">Order</th></cfif>--->
	      <th><a href="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&topid=#tempEncodeForURL(rc.topid)#&parentid=#tempEncodeForURL(rc.parentid)#&moduleid=#rc.moduleID#&sortBy=display&sortDirection=#displayDirection#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.display')#</a></th>
	      <th><a href="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&topid=#tempEncodeForURL(rc.topid)#&parentid=#tempEncodeForURL(rc.parentid)#&moduleid=#rc.moduleID#&sortBy=lastUpdate&sortDirection=#lastUpdatedDirection#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.lastupdated')#</a></th>
	      <th class="actions">&nbsp;</th>
	    </tr>
		</thead>
		</cfoutput>
          ...
          <td class="actions">
			<ul class="#lcase(rc.rstop.type)#">
				...
				<cfif (((rc.locking neq 'all') or (rc.parentid eq '#rc.topid#' and rc.locking eq 'none')) and (verdict eq 'editor') and not rc.rsTop.isLocked eq 1) and not isLockedBySomeoneElse>
					<li class="delete"><a title="#application.rbFactory.getKeyValue(session.rb,'sitemanager.content.delete')#" href="./?muraAction=cArch.update&contentid=#rc.rstop.ContentID#&type=#rc.rstop.type#&action=deleteall&topid=#tempEncodeForURL(rc.topid)#&siteid=#tempEncodeForURL(rc.siteid)#&moduleid=#rc.moduleid#&parentid=#tempEncodeForURL(rc.parentid)##rc.$.renderCSRFTokens(context=rc.rstop.contentid & 'deleteall',format='url')#" onClick="return confirmDialog('#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,'sitemanager.content.deletecontentconfirm'))#',this.href)"><i class="icon-remove-sign"></i></a></li>
    ...
    <cfoutput> 
		...
		<ul>
		 <cfif rc.nextN.currentpagenumber gt 1>
					...
					<li>	
				 	<a href="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&moduleid=#rc.moduleid#&topid=#tempEncodeForURL(rc.topid)#&startrow=#rc.nextN.previous#&sortBy=#rc.sortBy#&sortDirection=#rc.sortDirection#&searchString=#rc.searchString#">&laquo;&nbsp;#application.rbFactory.getKeyValue(session.rb,'sitemanager.prev')#</a>
				 	</li>
				 </cfif>
				<cfloop from="#rc.nextn.firstPage#"  to="#rc.nextn.lastPage#" index="i">
					...
					<cfelse> 
						<li>
						<a href="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&moduleid=#rc.moduleid#&topid=#tempEncodeForURL(rc.topid)#&startrow=#evaluate('(#i#*#rc.nextn.recordsperpage#)-#rc.nextn.recordsperpage#+1')#&sortBy=#rc.sortBy#&sortDirection=#rc.sortDirection#&searchString=#rc.searchString#">#i#</a></li>
					</cfif>
				</cfloop>
				<cfif rc.nextN.currentpagenumber lt rc.nextN.NumberOfPages>
					<li>
						<a href="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&moduleid=#rc.moduleid#&topid=#tempEncodeForURL(rc.topid)#&startrow=#rc.nextN.next#&sortBy=#rc.sortBy#&sortDirection=#rc.sortDirection#&searchString=#rc.searchString#">#application.rbFactory.getKeyValue(session.rb,'sitemanager.next')#&nbsp;&raquo;</a>
					</li>
				</cfif>
				</ul>
				</div>
...
<div class="sidebar span3">
	...
	<form class="form-inline" novalidate="novalidate" id="filterByTitle" action="index.cfm" method="get">
		...
		<h3>#application.rbFactory.getKeyValue(session.rb,"sitemanager.keywords")#</h3>
	     <input type="text" name="searchString" id="searchString" value="#tempEncodeForHTMLAttribute(rc.searchString)#" class="text" size="20">
	  	</div>
				...
				<div class="module well" id="mura-filter-tags">
					...
					<div id="#g#tags" class="tagSelector">
						<cfloop list="#$.event('#g#tags')#" index="i">
							<span class="tag">
							#tempEncodeForHTML(i)# <a><i class="icon-remove-sign"></i></a>
							<input name="#g#tags" type="hidden" value="#tempEncodeForHTMLAttribute(i)#">
							</span>
						</cfloop>
...
<cfif not len(crumbdata[1].siteid)>
  <cflocation url="./?muraAction=cArch.list&siteid=#tempEncodeForURL(rc.siteid)#&moduleid=00000000000000000000000000000000000&topid=00000000000000000000000000000000001" addtoken="false"/>
</cfif>
</cfsilent>
...
function initFlatViewArgs(){
	return {siteid:'#tempEncodeForJavascript(session.siteID)#', 
			moduleid:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].moduleid)#', 
			sortby:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].sortby)#', 
			sortdirection:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].sortdirection)#', 
			page:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].page)#',	
			tags:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].tags)#',
			categoryid:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].categoryid)#',
			lockid:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].lockid)#',
			type:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].type)#',
			subtype:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].subtype)#',
			report:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].report)#',
			keywords:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].keywords)#',
			filtered: '#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"].filtered)#'
			<cfif len($.siteConfig('customTagGroups'))>
			<cfloop list="#$.siteConfig('customTagGroups')#" index="g" delimiters="^,">
			,#g#tags:'#tempEncodeForJavascript(session.flatViewArgs["#session.siteID#"]["#g#tags"])#'	
			</cfloop>	
			</cfif>
			};
}
...
var newContentMenuTitle='#tempEncodeForJavascript(application.rbFactory.getKeyValue(session.rb,"sitemanager.selectcontenttype"))#';
...
function initSiteManagerTabContent(index){
	...
	switch(index){
		...
		if (!archViewLoaded) {
			...
			siteManager.loadSiteManager('#tempEncodeForJavascript(rc.siteID)#', '#tempEncodeForJavascript(rc.topid)#', '#tempEncodeForJavascript(rc.moduleid)#', '#tempEncodeForJavascript(rc.sortby)#', '#tempEncodeForJavascript(rc.sortdirection)#', '#tempEncodeForJavascript(rc.ptype)#', '#tempEncodeForJavascript(rc.startrow)#');
			archViewLoaded = true;
		}
		break;
		case 1:
