...
<cfoutput>
	...
	<tr>
		<td><cfif application.permUtility.getModulePerm('00000000000000000000000000000000008','#rc.siteid#')><a href="./?muraAction=cPublicUsers.advancedSearch&siteid=#encodeForURL(rc.siteid)#&param=1&paramField1=#encodeForURL('tusers.created^date')#&paramCondition1=GTE&paramCriteria1=#encodeForURL(rc.startDate)#&paramRelationship1=and&inActive=">#application.rbFactory.getKeyValue(session.rb,"dashboard.memberregistrations")#</a><cfelse>#application.rbFactory.getKeyValue(session.rb,"dashboard.memberregistrations")#</cfif><span> (#application.rbFactory.getResourceBundle(session.rb).messageFormat(application.rbFactory.getKeyValue(session.rb,"dashboard.span"),rc.span)#)</span></td>
