...
<cfif comments.recordCount() and comments.pageCount() gt 1>
	...
	<ul>
		<cfif comments.getPageIndex() gt 1> 
			<a href="./?muraAction=cDashboard.recentComments&page=#evaluate('comments.getPageIndex()-1')#&siteid=#encodeForURL(rc.siteid)#"><li>&laquo;&nbsp;#application.rbFactory.getKeyValue(session.rb,"dashboard.session.prev")#</a></li>
			</cfif>
		<cfloop from="1"  to="#comments.pageCount()#" index="i">
			<cfif comments.getPageIndex() eq i>
				<li class="active"> <a href="##">#i#</a></li> 
			<cfelse> 
				<li><a href="./?muraAction=cDashBoard.recentComments&page=#i#&siteid=#encodeForURL(rc.siteid)#">#i#</a>
				</li>
			</cfif>
		</cfloop>
		<cfif comments.getPageIndex() lt comments.pageCount()>
			<li><a href="./?muraAction=cDashboard.recentComments&page=#evaluate('comments.getPageIndex()+1')#&siteid=#encodeForURL(rc.siteid)#">#application.rbFactory.getKeyValue(session.rb,"dashboard.session.next")#&nbsp;&raquo;</a></li>
		</cfif>
	</ul>
	</div>
</cfif>	
