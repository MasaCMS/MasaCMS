...
<dl class="oneColumn">
		...
		<dd><select name="subtype" class="dropdown" onchange="resetExtendedAttributes('#request.userBean.getUserID()#','2',this.value,'#application.settingsManager.getSite(request.userBean.getSiteID()).getPublicUserPoolID()#','#application.configBean.getContext()#');">
...
<div class="page_aTab">
 ...
 <cfif rsSubTypes.recordcount>
		<div class="page_aTab">
			...
			loadExtendedAttributes('#request.userbean.getUserID()#','#request.userbean.getType()#','#request.userBean.getSubType()#','#application.settingsManager.getSite(request.userBean.getSiteID()).getPrivateUserPoolID()#','#application.configBean.getContext()#');
			</script>	
	</div>
