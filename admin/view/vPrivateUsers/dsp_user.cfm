...
<div class="page_aTab">
	<dl class="oneColumn">
		...
		<dd><select name="subtype" class="dropdown" onchange="resetExtendedAttributes('#request.userBean.getUserID()#','2',this.value,'#application.settingsManager.getSite(attributes.siteID).getPrivateUserPoolID()#','#application.configBean.getContext()#');">
...
<dd><input  name="password2" autocomplete="off" type="password" value="" class="text"  message="#application.rbFactory.getKeyValue(session.rb,'user.passwordconfirm')#"></dd>    
	...
	<div class="page_aTab">
		...
		<cfelse>
		<dt class="first"><ul id="navTask"><li><a href="index.cfm?fuseaction=cPrivateUsers.editAddress&userID=#attributes.userid#&siteid=#attributes.siteid#&routeID=#attributes.routeid#&addressID=&returnURL=#urlencodedformat(cgi.query_string)#">#application.rbFactory.getKeyValue(session.rb,'user.addnewaddress')#</a></li></ul></dt>
		...
		<tr>
			...
			<td>
			<cfif rsAddresses.addressName neq ''><a title="Edit" href="index.cfm?fuseaction=cPrivateUsers.editAddress&userID=#attributes.userid#&siteid=#attributes.siteid#&routeID=#attributes.routeid#&addressID=#rsAddresses.addressID#&returnURL=#urlencodedformat(cgi.query_string)#"><strong>#rsAddresses.addressName#</strong></a><br/></cfif>
	...
	<cfif rsSubTypes.recordcount>
		<div class="page_aTab">
			...
			loadExtendedAttributes('#request.userbean.getUserID()#','#request.userbean.getType()#','#request.userBean.getSubType()#','#application.settingsManager.getSite(attributes.siteID).getPrivateUserPoolID()#','#application.configBean.getContext()#');
			</script>	
	</div>
	...
	<div class="page_aTab">
		...
		<cfif attributes.userid eq ''>
           ...
           <cfelse>
            	<a class="submit" href="javascript:;" onclick="return submitForm(document.forms.form1,'delete','#jsStringFormat(application.rbFactory.getKeyValue(session.rb,'user.deleteuserconfirm'))#');"><span>#application.rbFactory.getKeyValue(session.rb,'user.delete')#</span></a> <a class="submit" href="javascript:;" onclick="return submitForm(document.forms.form1,'update');"><span>#application.rbFactory.getKeyValue(session.rb,'user.update')#</span></a>
