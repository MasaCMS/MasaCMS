...
<cfset request.rslist = application.userManager.getSearch(attributes.search,attributes.siteid,1) >
<cfif request.rslist.recordcount eq 1>
<cflocation url="index.cfm?fuseaction=cPublicUsers.editUser&userid=#request.rslist.userid#&siteid=#attributes.siteid#" addtoken="false">
<cfabort>
</cfif>
<cfset request.nextn = application.utility.getNextN(request.rsList,15,attributes.startrow) >
<!--- do action="vPublicUsers.search" --->
...
<cfif not isDefined("request.error")><cfset request.error = "#structnew()#" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vPublicUsers/dsp_search.cfm"></cfoutput>
...
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_search.cfm in circuit vPublicUsers which does not exist (from fuseaction vPublicUsers.search).">
