...
<!--- fuseaction: advancedSearch --->
<cftry>
<cfset myFusebox.thisPhase = "requestedFuseaction">
<cfset myFusebox.thisCircuit = "cPrivateUsers">
<cfset myFusebox.thisFuseaction = "advancedSearch">
...
<cfif not (isUserInRole('Admin;#application.settingsManager.getSite(attributes.siteid).getPrivateUserPoolID()#;0') or  isUserInRole('S2'))  and not (listFindNoCase('cPrivateUsers.editAddress,cPrivateUsers.updateAddress',attributes.fuseaction) and  attribute.userID eq listFirst(getAuthUser(),'^'))>
...
<cfset request.rslist = application.userManager.getAdvancedSearch(attributes,attributes.siteid,0) >
<cfif request.rslist.recordcount eq 1>
<cflocation url="index.cfm?fuseaction=cPrivateUsers.editUser&userid=#request.rslist.userid#&siteid=#attributes.siteid#" addtoken="false">
<cfabort>
</cfif>
<cfset request.nextn = application.utility.getNextN(request.rsList,15,attributes.startrow) >
<!--- do action="vPrivateUsers.search" --->
...
<cfif not isDefined("request.error")><cfset request.error = "#structnew()#" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vPrivateUsers/dsp_search.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 14 and right(cfcatch.MissingFileName,14) is "dsp_search.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_search.cfm in circuit vPrivateUsers which does not exist (from fuseaction vPrivateUsers.search).">
