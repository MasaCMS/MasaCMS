...
<cfset request.emailBean = application.emailManager.read(attributes.emailid) >
<cfset request.rsPrivateGroups = application.emailManager.getPrivateGroups(attributes.siteid) >
<cfset request.rsPublicGroups = application.emailManager.getPublicGroups(attributes.siteid) >
<cfset request.rsMailingLists = application.emailManager.getMailingLists(attributes.siteid) >
...
<cfif not isDefined("session.emaillist.dontshow")><cfset session.emaillist.dontshow = "1" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vEmail_Broadcaster/dsp_form.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 12 and right(cfcatch.MissingFileName,12) is "dsp_form.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_form.cfm in circuit vEmail which does not exist (from fuseaction vEmail.Edit).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
</cfsavecontent>
