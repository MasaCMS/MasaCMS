...
<!--- fuseaction: viewReportByPlacement --->
<cftry>
<cfset myFusebox.thisPhase = "requestedFuseaction">
<cfset myFusebox.thisCircuit = "cAdvertising">
<cfset myFusebox.thisFuseaction = "viewReportByPlacement">
...
<cfset request.creativeBean = application.advertiserManager.readCreative(request.placementBean.getCreativeID()) >
<cfset request.adZoneBean = application.advertiserManager.readAdZone(request.placementBean.getAdZoneID()) >
<cfset application.advertiserManager.getReportDataByPlacement(attributes,request.placementBean) >
<!--- do action="vAdvertising.viewReportByPlacement" --->
...
<cfif not isDefined("attributes.keywords")><cfset attributes.keywords = "" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vAdvertising/dsp_viewReportByPlacement.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 29 and right(cfcatch.MissingFileName,29) is "dsp_viewReportByPlacement.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_viewReportByPlacement.cfm in circuit vAdvertising which does not exist (from fuseaction vAdvertising.viewReportByPlacement).">
