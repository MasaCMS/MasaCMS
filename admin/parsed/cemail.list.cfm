...
<cfset request.rsList = application.emailManager.getList(attributes) >
...
<cfif not isDefined("session.emaillist.dontshow")><cfset session.emaillist.dontshow = "1" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vEmail_Broadcaster/dsp_list.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 12 and right(cfcatch.MissingFileName,12) is "dsp_list.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_list.cfm in circuit vEmail which does not exist (from fuseaction vEmail.list).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
</cfsavecontent>
