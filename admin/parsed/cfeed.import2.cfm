...
<cfset myFusebox.thisFuseaction = "import2">
...
<cfset request.import = application.feedManager.import(attributes) >
<!--- do action="vFeed.import2" --->
<cfset myFusebox.thisCircuit = "vFeed">
<cfsavecontent variable="fusebox.layout">
<cftry>
<cfoutput><cfinclude template="../view/vFeed/dsp_import2.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 15 and right(cfcatch.MissingFileName,15) is "dsp_import2.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_import2.cfm in circuit vFeed which does not exist (from fuseaction vFeed.import2).">
