...
<cfset myFusebox.thisFuseaction = "loadSite">
...
<!--- do action="vFeed.loadSite" --->
<cfset myFusebox.thisCircuit = "vFeed">
<cfsavecontent variable="fusebox.layout">
<cftry>
<cfoutput><cfinclude template="../view/vFeed/ajax/dsp_loadSite.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 21 and right(cfcatch.MissingFileName,21) is "ajax/dsp_loadSite.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse ajax/dsp_loadSite.cfm in circuit vFeed which does not exist (from fuseaction vFeed.loadSite).">
