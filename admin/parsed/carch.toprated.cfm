...
<cfset myFusebox.thisFuseaction = "topRated">
...
<cfset request.rsList = application.raterManager.getTopRated(attributes.siteid,attributes.size) >
<cfset request.nextn = application.utility.getNextN(request.rsList,30,attributes.startrow) >
<!--- do action="vArch.topRated" --->
...
<cftry>
<cfoutput><cfinclude template="../view/vArchitecture/dsp_topRated.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 16 and right(cfcatch.MissingFileName,16) is "dsp_topRated.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_topRated.cfm in circuit vArch which does not exist (from fuseaction vArch.topRated).">
