...
<cfset application.contentManager.getReportData(attributes,request.contentBean) >
...
<cfif not isDefined("attributes.locking")><cfset attributes.locking = "none" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vArchitecture/dsp_viewreport.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 18 and right(cfcatch.MissingFileName,18) is "dsp_viewreport.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_viewreport.cfm in circuit vArch which does not exist (from fuseaction vArch.viewreport).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
</cfsavecontent>
