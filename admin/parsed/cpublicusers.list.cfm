...
<cfset request.rsGroups = application.userManager.getUserGroups('#attributes.siteid#',1) >
<!--- do action="vPublicUsers.list" --->
...
<cfif not isDefined("request.error")><cfset request.error = "#structnew()#" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vPublicUsers/dsp_list.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 12 and right(cfcatch.MissingFileName,12) is "dsp_list.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_list.cfm in circuit vPublicUsers which does not exist (from fuseaction vPublicUsers.list).">
