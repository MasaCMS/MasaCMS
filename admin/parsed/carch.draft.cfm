...
<cfset myFusebox.thisFuseaction = "draft">
...
<cfset request.rsList = application.contentManager.getDraftList(attributes.siteid) >
...
<cfif not isDefined("attributes.locking")><cfset attributes.locking = "none" /></cfif>
<cftry>
<cfoutput><cfinclude template="../view/vArchitecture/dsp_draftlist.cfm"></cfoutput>
<cfcatch type="missingInclude"><cfif len(cfcatch.MissingFileName) gte 17 and right(cfcatch.MissingFileName,17) is "dsp_draftlist.cfm">
<cfthrow type="fusebox.missingFuse" message="missing Fuse" detail="You tried to include a fuse dsp_draftlist.cfm in circuit vArch which does not exist (from fuseaction vArch.draft).">
<cfelse><cfrethrow></cfif></cfcatch></cftry>
</cfsavecontent>
