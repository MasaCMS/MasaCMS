	                    ...
	                    <a class="dropdown-toggle" data-toggle="dropdown" href="##"><i class="icon-cogs"></i> #application.rbFactory.getKeyValue(session.rb,"layout.settings")#
	                      <b class="caret"></b>
		                    ...
		                    <cfif listFind(session.mura.memberships,'S2')>
		                     	...
		                     	<li>
		                     		<a <cfif not isBoolean(application.configBean.getAllowAutoUpdates()) or application.configBean.getAllowAutoUpdates()><!--- <cfif $.globalconfig('dbtype') eq "mssql"> --->href="##" onclick="confirmDialog('WARNING: Do not update your core files unless you have backed up your current Mura install.<cfif application.configBean.getDbType() eq "mssql">\n\nIf your are using MSSQL you must uncheck Maintain Connections in your CF administrator datasource settings before proceeding. You may turn it back on after the update is complete.</cfif>',function(){actionModal('index.cfm?muraAction=cSettings.list&action=updateCore')});return false;"<!--- </cfif> ---></cfif>>
