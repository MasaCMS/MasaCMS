	              ...
	              <!--- Global Settings --->
	               <cfif session.siteid neq '' and session.mura.isLoggedIn>
	               		<cfif listFind(session.mura.memberships,'S2')>               
	                  		<li id="navGlobalSettings" class="dropdown">
			                    ...
			                    <li>
		                     	...
		                     	<li><a href="#application.configBean.getContext()#/admin/?#esapiEncode('url',application.appreloadkey)#&reload=#esapiEncode('url',application.appreloadkey)#" onclick="return actionModal(this.href);"><i class="icon-refresh"></i> #application.rbFactory.getKeyValue(session.rb,"layout.reloadapplication")#</a></li>
					...
					<li id="navEditProfile" class="dropdown">
	                 	<a class="dropdown-toggle" data-toggle="dropdown" href="##"><i class="icon-user"></i> #esapiEncode("html","#session.mura.fname# #session.mura.lname#")#
	                 		<b class="caret"></b></a>
		                 <ul class="dropdown-menu">
		                 <li>
	 			...
	 			<ul>
		 				...
		 				<ul id="select-site-ul" class="dropdown-menu ui-front">
		 				    ...
		 				    <cfloop query="theSiteList" startrow="1" endrow="100">
		 				      <li<cfif session.siteID eq theSiteList.siteID> class="active"</cfif>>
		 				        <a href="#baseURL#&amp;siteID=#theSiteList.siteID#" title="#esapiEncode('html_attr',theSiteList.site)#"><i class="icon-globe"></i> #esapiEncode('html',theSiteList.site)#</a>
		 				      </li>
		 				    </cfloop>
		 					...
		 					<ul class="dropdown-menu">
		 						<cfif listFind(session.mura.memberships,'Admin;#application.settingsManager.getSite(session.siteid).getPrivateUserPoolID()#;0') or listFind(session.mura.memberships,'S2')>
					 				...
					 				<cfif application.settingsManager.getSite(session.siteid).getextranet() and  application.permUtility.getModulePerm("00000000000000000000000000000000008","#session.siteid#")>
						 				 ...
						 				 <li<cfif request.action eq "core:cpublicusers.edituser" and not len(rc.userID)> class="active"</cfif>><a href="./?muraAction=cPublicUsers.edituser&siteid=#esapiEncode('url',session.siteid)#&userid="><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,'user.addmember')#</a></li>
						 				<li<cfif request.action eq "core:cpublicusers.editgroup" and not len(rc.userID)> class="active"</cfif>><a href="./?muraAction=cPublicUsers.editgroup&siteid=#esapiEncode('url',session.siteid)#&userid="><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,'user.addmembergroup')#</a></li>
			 					   	</cfif>
		 					  </ul>			
	 					</li>
	 				</cfif>
	 				...
	 				<cfif listFind(session.mura.memberships,'Admin;#application.settingsManager.getSite(session.siteid).getPrivateUserPoolID()#;0') or listFind(session.mura.memberships,'S2')>
		 				...
		 				<ul class="dropdown-menu">
		 					...
		 					<a href="./?muraAction=cExtend.listSubTypes&siteid=#esapiEncode('url',session.siteid)#"><i class="icon-wrench"></i> Class Extension Manager</a>
			 					<ul class="dropdown-menu">
			 						...
			 						<!--- --->
				 					 <li><a href="#application.configBean.getContext()#/admin/?muraAction=cExtend.editSubType&subTypeID=&siteid=#esapiEncode('url',session.siteid)#"><i class="icon-plus-sign"></i> #application.rbFactory.getKeyValue(session.rb,"layout.addclassextension")#</a></li>
				 					<cfif rsExts.recordcount>
				 						<li class="divider"></li>
				 					</cfif>
				 					<cfloop query="rsExts">
				 						<li><a href="#application.configBean.getContext()#/admin/?muraAction=cExtend.listSets&subTypeID=#rsExts.subtypeID#&siteid=#esapiEncode('url',session.siteid)#">
				 							<i class=<cfif len(exp)>"#evaluate(exp)#"<cfelse>"icon-cog"</cfif>></i>				 								
				 							<cfif rsExts.type eq 1>Group<cfelseif rsExts.type eq 2>User<cfelse>#esapiEncode('html',rsExts.type)#</cfif>/#esapiEncode('html',rsExts.subtype)#	
				 						</a></li>
				 					</cfloop>
			 					</ul>
		 					</li>
							...
							<li>
		 					...
		 					<cfif len(rc.siteBean.getExportLocation()) and directoryExists(rc.siteBean.getExportLocation())>
		 						<li>
									<a href="##" onclick="confirmDialog('Export static HTML files to #esapiEncode("javascript","'#rc.siteBean.getExportLocation()#'")#.',function(){actionModal('./?muraAction=csettings.exportHTML&siteID=#rc.siteBean.getSiteID()#')});return false;"><i class="icon-cog"></i> Export Static HTML (BETA)</a>
								</li>
							</cfif>
