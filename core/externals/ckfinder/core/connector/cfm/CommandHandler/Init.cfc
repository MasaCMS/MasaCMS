...
<cffunction access="public" name="buildXml" hint="send XML response" returntype="boolean" description="send response" output="true">
	...
	<cfscript>
		...
		if (not structkeyexists(THIS.config,"enableCsrfProtection") or THIS.config.enableCsrfProtection) {
			nodeConnectorInfo.xmlAttributes["csrfProtection"] = "true";
		}
		else {
			nodeConnectorInfo.xmlAttributes["csrfProtection"] = "false";
		}
