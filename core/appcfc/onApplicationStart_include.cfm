...
if ( application.setupComplete ) {
	...
	if ( application.configBean.getCreateRequiredDirectories() ) {
		...
		if ( !fileExists(variables.basedir & "/modules/cfformprotect/cffp.ini.cfm") ) {
			application.serviceFactory.getBean("fileWriter").copyFile(source="#variables.basedir#/core/templates/cffp.ini.template.cfm", destination="#variables.basedir#/core/resources/cfformprotect/cffp.ini.cfm");
		}
	}
