...
component extends="mura.cfobject" output="false" {
	...
	public function getLastSessionDate(urlToken, originalUrlToken, beforeDate) {
		var rs = queryExecute(
			sql = "select max(entered) as lastRequest
	...
	public function getUserFromSessionQuery(rsSession) {
		var rs = queryExecute(
			sql = "select fname,lname from arguments.rsSession where userID > ''"
	...
	public function getUserAgentFromSessionQuery(rsSession) {
		var rs = queryExecute(
			sql = "select user_agent from arguments.rsSession where user_agent > ''"
			,options = {
				dbtype = 'query'
			}
		);
