...
component extends="mura.bean.beanFeed" entityName="feed" table="tcontentfeeds" output="false" hint="This provides content feed bean functionality" {
	...
	public function setAltTable(any altTable) output=false {
		if ( len(arguments.altTable) ) {
			...
			// Allow schema-qualified names (e.g., dbo.tcontent) up to 2 parts
			var sanitized = reReplace(arguments.altTable, "[^a-zA-Z0-9_.]", "", "all");

			// Split by dots and validate each part
			var parts = listToArray(sanitized, '.');
			var isValid = true;

			// Maximum 2 parts: schema.table
			if (arrayLen(parts) > 2 || arrayLen(parts) == 0) {
				isValid = false;
			} else {
				// Each part must be a valid SQL identifier: starts with letter/underscore, contains only alphanumeric and underscore
				for (var part in parts) {
					if (!reFind("^[a-zA-Z_][a-zA-Z0-9_]*$", part) || len(part) > 64) {
						isValid = false;
						break;
					}
				}
			...
			//check altTable exists, otherwise default to tcontent
			if ( isValid && len(sanitized) && getBean('dbUtility').tableExists(sanitized) ) {
				variables.instance.altTable = sanitized;
			} else {
				variables.instance.altTable = "";
			}
		}
