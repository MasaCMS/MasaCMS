component extends="mura.cfobject" hint="This provides JSON/REST API functionality" {
	...
	function packageIteratorArray(iterator,itArray,method,expanded=false){
		...
		if(iterator.getEntityName() != 'bean'){
		...
		if(arguments.expanded){
			...
			for(var p in queryParams){
				...
				if(structKeyExists(params,p)){
					...
					if(!listFindNoCase('_cacheid,distinct,fields,entityname,method,maxItems,pageIndex,itemsPerPage,sortBy,sortDirection,contentpoolid,shownavonly,showexcludesearch,includehomepage',p)){
						if(propName == 'sort'){
