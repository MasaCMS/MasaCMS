	...
	public function getExtendedAttributesQuery(name="") output=false {
		...
		if ( Len(arguments.name) ) {
			if ( rsDefinitions.recordcount ) {
				rsExtendSet = queryExecute(
					sql = "SELECT DISTINCT extendsetid FROM rsDefinitions WHERE extendsetname = :name"
					, params = {
						name: {
			...
			if ( rsExtendSet.recordcount ) {
				rsAttributes = queryExecute(
					sql = "SELECT * FROM rsData WHERE extendsetid = :id"
					, params = {
						id: {
