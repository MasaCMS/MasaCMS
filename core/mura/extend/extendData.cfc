...
<cffunction name="getAttribute" output="false">
	...
	<cfset var tempDate="">
	<cfset var index=0>
	<cfset var nameKey="name_" & hash(lcase(arguments.key),'CFMX_COMPAT')>
...
<cffunction name="loadData" output="false">
	...
	<cfloop query='rsExtended'>
		...
		<cfset variables.instance.nameLookup["name_" & hash(lcase(rsExtended.name),'CFMX_COMPAT')]=rsExtended.currentRow>
	</cfloop>
