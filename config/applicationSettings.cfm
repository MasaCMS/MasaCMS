...
<cfif request.muraInDocker and not fileExists(baseDir & "/config/cfapplication.cfm")>
	...
	<cftry>
	<cffile action="copy" source="#baseDir#/config/docker/global/cfapplication.cfm" destination="#baseDir#/config/cfapplication.cfm" mode="777">
	<cfcatch>
		<cffile action="copy" source="#baseDir#/config/docker/global/cfapplication.cfm" destination="#baseDir#/config/cfapplication.cfm">
	</cfcatch>
	</cftry>
	<cfset commitTracePoint(variables.tracePoint)>
